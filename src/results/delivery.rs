...
pub enum DeliveryStatus {
    Pending,
    InProgress { bytes_sent: usize, total_bytes: usize },
    Completed,
    Failed(String),
}
...
impl P2PDeliveryService {
    ...
    pub async fn deliver_result(
        ...
        if !self.is_peer_connected(&client_peer) {
            ...
            tx.send(DeliveryProgress {
                job_id: job_id.clone(),
                status: DeliveryStatus::InProgress { bytes_sent: 0, total_bytes },
                timestamp: Instant::now(),
            }).await?;
        }
        ...
        tokio::spawn(async move {
            ...
            for chunk in buffer.chunks(chunk_size) {
                ...
                let _ = tx.send(DeliveryProgress {
                    job_id: job_id.clone(),
                    status: DeliveryStatus::InProgress { bytes_sent, total_bytes },
                    timestamp: Instant::now(),
                }).await;
