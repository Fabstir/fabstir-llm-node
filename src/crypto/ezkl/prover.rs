...
impl EzklProver {
    ...
    fn generate_real_proof(&mut self, witness: &Witness, timestamp: u64) -> EzklResult<ProofData> {
        ...
        // Generate proof using in-process LocalProver (no external r0vm needed)
        tracing::info!("🔨 Running Risc0 prover in-process (this may take 2-3 seconds on CPU)...");
        let prover: LocalProver = LocalProver::new("prove");

        let prove_info = Prover::prove(&prover, env, COMMITMENT_GUEST_ELF).map_err(|e| {
        ...
        tracing::debug!("📦 Serializing receipt to bytes");
        ...
        if proof_bytes.len() < 100_000 {
            tracing::warn!(
                "⚠️  Proof size ({} bytes) is smaller than expected for STARK proofs (> 100KB)",
                proof_bytes.len()
            );
        }
        if proof_bytes.len() > 500_000 {
            tracing::warn!(
                "⚠️  Proof size ({} bytes) is larger than expected for STARK proofs (< 500KB)",
                proof_bytes.len()
            );
        }
