...
impl Default for JobVerificationConfig {
    fn default() -> Self {
        Self {
            ...
            marketplace_address: std::env::var("JOB_MARKETPLACE_FAB_WITH_S5_ADDRESS")
...
pub struct JobVerifier {
    ...
    web3_client: Option<Arc<Web3Client>>,
    cache: Arc<RwLock<HashMap<u64, CacheEntry>>>,
}

impl JobVerifier {
    /// Create new job verifier
    pub async fn new(config: JobVerificationConfig, rpc_url: String) -> Result<Self> {
        let web3_client = if config.blockchain_verification {
            ...
            Some(Arc::new(Web3Client::new(web3_config).await?))
        } else {
            None
        };
    ...
    /// Verify a job by ID
    pub async fn verify_job(&self, job_id: u64) -> Result<JobDetails> {
        // If disabled, return mock job
        if !self.config.enabled {
            return Ok(self.create_mock_job(job_id));
        }
        
        // Check cache first
        if let Some(details) = self.get_cached_job(job_id).await {
            debug!("Job {} found in cache", job_id);
            return Ok(details);
        }
        
        // Verify from blockchain
        if let Some(ref client) = self.web3_client {
            let job = self.fetch_job_from_blockchain(client, job_id).await?;
            let details = self.convert_job_to_details(job_id, job)?;
            ...
            // Fallback to mock if no blockchain client
            Ok(self.create_mock_job(job_id))
        }
    }
    ...
    pub async fn verify_payment_escrowed(&self, job: &JobDetails) -> Result<bool> {
        ...
        if let Some(ref client) = self.web3_client {
            // Check escrow contract for payment
            // This would call the actual escrow contract
            debug!("Verifying payment for job {}: {} wei", job.job_id, job.payment_amount);
    ...
    pub async fn create_claim_message(&self, job_id: u64, host_address: &str) -> String {
        format!("Claim job {} as host {}", job_id, host_address)
    }
    ...
    pub async fn verify_claim_signature(
        ...
        if let Some(ref client) = self.web3_client {
            let message = self.create_claim_message(job_id, host_address).await;
    ...
    /// Get job metadata
    pub async fn get_job_metadata(&self, job_id: u64) -> Result<HashMap<String, String>> {
        let job = self.verify_job(job_id).await?;
    ...
    /// Batch verify jobs
    pub async fn batch_verify_jobs(&self, job_ids: Vec<u64>) -> Result<Vec<JobDetails>> {
        let mut results = Vec::new();
        
        for job_id in job_ids {
            match self.verify_job(job_id).await {
    ...
    fn convert_job_to_details(&self, job_id: u64, job: Job) -> Result<JobDetails> {
        Ok(JobDetails {
            job_id,
    ...
    fn create_mock_job(&self, job_id: u64) -> JobDetails {
        JobDetails {
            job_id,
