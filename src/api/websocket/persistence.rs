...
use super::storage_trait::{SessionStorage, FileStorage};
...
impl SessionPersistence {
                ...
                Ok(session_ids) => {
                    for session_id in session_ids {
                        match self.storage.load_session(chain_id, &session_id).await {
                            ...
                            Err(e) => {
                                warn!("Failed to load session {} from chain {}: {}",
    ...
    pub async fn create_chain_backup(&self, chain_id: u64) -> Result<String> {
        ...
        info!("Created backup {} for chain {} with {} sessions",
    ...
    pub async fn restore_from_backup(&self, chain_id: u64, backup_id: &str) -> Result<usize> {
        ...
        info!("Restored {} sessions from backup {} for chain {}",
              restored, backup_id, chain_id);
        Ok(restored)
    }

    /// Migrate a session to a different chain
    pub async fn migrate_session_chain(&self, session_id: &str, from_chain: u64, to_chain: u64) -> Result<()> {
        ...
        info!("Migrated session {} from chain {} to chain {}",
    ...
    pub async fn auto_backup_if_needed(&mut self, chain_id: u64, last_backup: Option<u64>) -> Result<Option<String>> {
