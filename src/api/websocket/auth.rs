...
impl Authenticator {
    ...
    pub async fn verify_token(&self, token: &str) -> AuthResult<SessionToken> {
        ...
        let session_token = store.get(token).ok_or(AuthError::InvalidToken)?;
    ...
    pub async fn decode_jwt(&self, jwt: &str) -> AuthResult<JwtClaims> {
        ...
        .map_err(|e| {
            ...
            if error_str.to_lowercase().contains("expired")
    ...
    pub async fn create_mfa_token(
        ...
        let mut token = self
            .create_session_token(session_id, job_id, vec![])
            .await?;
...
impl SignatureVerifier {
    pub async fn new(config: SignatureConfig) -> Result<Self> {
        ...
        let (signing_key, verifying_key) = if let Some(priv_key_hex) = &config.private_key {
            ...
            let secret_bytes =
                hex::decode(priv_key_hex).map_err(|_| anyhow!("Invalid private key hex"))?;

            let secret_bytes_array: [u8; 32] = secret_bytes
                .try_into()
            ...
            let public_bytes =
                hex::decode(pub_key_hex).map_err(|_| anyhow!("Invalid public key hex"))?;
