...
impl LivenessCheck {
    ...
    pub async fn record_heartbeat(&self, component: &str) {
        self.last_heartbeats.write().await
    ...
    pub async fn get_status(&self) -> LivenessStatus {
        ...
        LivenessStatus {
            last_websocket_heartbeat: heartbeats.get("websocket")
                .map(|t| t.elapsed())
                .unwrap_or(Duration::ZERO),
            last_inference_heartbeat: heartbeats.get("inference")
                .map(|t| t.elapsed())
                .unwrap_or(Duration::ZERO),
        }
    }
...
impl HealthChecker {
    ...
    pub async fn record_failure(&self, component: &str) {
        if let Some(breaker) = &self.circuit_breaker {
            ...
            let component_failures = failures.entry(component.to_string()).or_insert_with(Vec::new);
            ...
            if component_failures.len() >= breaker.failure_threshold {
                ...
                tokio::spawn(async move {
                    ...
                    state_clone.write().await.insert(component_clone, CircuitState::HalfOpen);
                });
            }
        }
    }
    ...
    pub async fn record_success(&self, component: &str) {
        if let Some(breaker) = &self.circuit_breaker {
            let mut states = breaker.state.write().await;
            let current_state = states.get(component).cloned().unwrap_or(CircuitState::Closed);
