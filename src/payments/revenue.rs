...
impl RevenueCalculator {
    ...
    pub async fn record_revenue(&self, revenue: Revenue) -> Result<()> {
        ...
        self.revenue_by_job.write().await.insert(revenue.job_id, revenue);
        Ok(())
    }
    ...
    pub async fn get_revenue_stats(&self) -> Result<RevenueStats> {
        ...
        let total_gross_revenue = history.iter()
            .map(|r| r.gross_amount)
            .fold(U256::zero(), |acc, amt| acc + amt);
        
        let total_net_revenue = history.iter()
            .map(|r| r.net_amount)
            .fold(U256::zero(), |acc, amt| acc + amt);
        
        let total_fees_paid = history.iter()
        ...
        let today_revenue = history.iter()
            .filter(|r| r.timestamp >= today_start)
            .map(|r| r.net_amount)
            .fold(U256::zero(), |acc, amt| acc + amt);
        
        let yesterday_revenue = history.iter()
...
mod tests {
    ...
    async fn test_basic_revenue_calculation() {
        ...
        assert_eq!(revenue.marketplace_fee, U256::from(5_000_000_000_000_000u64)); // 5%
