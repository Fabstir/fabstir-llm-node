...
impl VectorDbClient {
    pub fn new(config: VectorDbConfig) -> Result<Self> {
        let _parsed_url =
            reqwest::Url::parse(&config.api_url).map_err(|e| anyhow!("Invalid URL: {}", e))?;
    ...
    pub async fn insert_vector_json(&self, mut vector_data: Value) -> Result<Value> {
        ...
        let response = self.client.post(&url).json(&vector_data).send().await?;
    ...
    pub async fn search_vectors(&self, mut search_query: Value) -> Result<Value> {
        ...
        if let Some(query_vector) = search_query.get("query_vector") {
            ...
            search_query
                .as_object_mut()
                .map(|obj| obj.remove("query_vector"));
        }
        ...
        let response = self.client.post(&url).json(&search_query).send().await?;
    ...
    pub async fn batch_insert(&self, mut batch_data: Value) -> Result<Value> {
        ...
        let response = self.client.post(&url).json(&batch_data).send().await?;
