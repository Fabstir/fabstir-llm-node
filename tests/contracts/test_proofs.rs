use fabstir_llm_node::contracts::{ProofSubmitter, ProofConfig, ProofData, ProofStatus};
use ethers::prelude::*;
use std::sync::Arc;
...
async fn test_proof_generation() {
    ...
    let proof_data = submitter.generate_proof(
        job_id,
        model_commitment,
        input_hash,
        output_hash,
...
async fn test_proof_submission() {
    ...
    submitter.set_wallet("0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80")
    ...
    // Submit proof
    let tx_hash = submitter.submit_proof(proof_data.clone())
        .await
        .expect("Failed to submit proof");
    
    // Wait for confirmation
    let receipt = web3_client.wait_for_confirmation(tx_hash)
        .await
    ...
    let proof_status = submitter.get_proof_status(job_id)
        .await
...
async fn test_proof_verification_monitoring() {
    ...
    match event {
        ProofEvent::ProofVerified { job_id: id, is_valid } => {
            assert_eq!(id, job_id);
            assert!(is_valid);
        }
...
async fn test_challenge_detection() {
    ...
    match event {
        ProofEvent::ProofChallenged { job_id: id, challenger, reason } => {
            assert_eq!(id, job_id);
...
async fn test_batch_proof_submission() {
    ...
    // Submit batch
    let tx_hash = submitter.submit_proof_batch(proofs)
        .await
...
async fn test_proof_storage_optimization() {
    ...
    // Compress and store
    let stored_proof = submitter.prepare_proof_for_submission(large_proof.clone())
        .await
...
async fn test_proof_timing_requirements() {
    ...
    let can_submit = submitter.check_proof_deadline(job_id)
        .await
    ...
    let can_submit_late = submitter.check_proof_deadline(late_job_id)
        .await
        .expect("Failed to check deadline");
...
async fn test_proof_resubmission_on_failure() {
    ...
    let result = submitter.submit_proof_with_retry(proof)
        .await;
...
async fn test_proof_validation_before_submission() {
    ...
    let is_valid = submitter.validate_proof(&valid_proof)
        .await
    ...
    let is_valid = submitter.validate_proof(&invalid_proof)
        .await
        .expect("Failed to validate proof");
...
async fn test_challenge_response_submission() {
    ...
    let tx_hash = submitter.submit_challenge_response(challenge_id, response_data)
        .await
...
async fn test_proof_fee_calculation() {
    ...
    let fee = submitter.calculate_verification_fee(proof_size)
        .await
    ...
    let large_fee = submitter.calculate_verification_fee(large_proof_size)
        .await
        .expect("Failed to calculate fee");
