...
use ethers::prelude::*;
use fabstir_llm_node::contracts::{PaymentConfig, PaymentStatus, PaymentVerifier, TokenInfo};
...
async fn test_escrow_deposit_verification() {
    ...
    let deposit = verifier
        .verify_escrow_deposit(job_id)
        .await
...
async fn test_token_balance_checking() {
    ...
    // Check ETH balance
    let eth_balance = verifier
        .get_token_balance("ETH", wallet_address)
        .await
    ...
    // Check USDC balance
    let usdc_balance = verifier
        .get_token_balance("USDC", wallet_address)
        .await
...
async fn test_payment_approval_verification() {
    ...
    let client_address = "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"
        .parse::<Address>()
        .unwrap();
    let amount = U256::from(100_000_000); // 100 USDC

    // Check if client has approved escrow
    let is_approved = verifier
        .check_token_approval("USDC", client_address, amount)
        .await
...
async fn test_multi_token_job_payments() {
    ...
    let eth_job =
        create_job_with_token(&web3_client, "ETH", U256::from(1_000_000_000_000_000u64)).await;

    // Verify both payments
    let usdc_deposit = verifier
        .verify_escrow_deposit(usdc_job)
        .await
        .expect("Failed to verify USDC deposit");

    let eth_deposit = verifier
        .verify_escrow_deposit(eth_job)
        .await
...
async fn test_payment_timeout_detection() {
    ...
    let is_timed_out = verifier
        .check_payment_timeout(job_id)
        .await
    ...
    let can_refund = verifier
        .can_request_refund(job_id)
        .await
...
async fn test_batch_payment_processing() {
    ...
    let results = verifier
        .process_batch_payments(&job_ids)
        .await
...
async fn test_payment_history_tracking() {
    ...
    let host_address = "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"
        .parse::<Address>()
        .unwrap();

    // Get payment history for host
    let history = verifier
        .get_payment_history(host_address, 30) // Last 30 days
        .await
