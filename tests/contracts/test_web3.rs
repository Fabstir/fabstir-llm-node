use fabstir_llm_node::contracts::{Web3Client, Web3Config, ChainConfig};
use ethers::prelude::*;
use std::sync::Arc;
...
async fn test_wallet_management() {
    ...
    assert_eq!(address, "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266".parse::<Address>().unwrap());
...
async fn test_contract_deployment_addresses() {
    ...
    let addresses = client.load_contract_addresses("deployments/localhost.json")
        .await
...
async fn test_gas_estimation() {
    let config = Web3Config::default();
    let mut client = Web3Client::new(config).await.expect("Failed to create Web3 client");
    
    // Set up wallet
    client.set_wallet("0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80")
        .expect("Failed to set wallet");
    
    // Estimate gas for a simple transfer
    let to = "0x70997970C51812dc3A010C7d01b50e0d17dc79C8".parse::<Address>().unwrap();
    let value = U256::from(1_000_000_000_000_000u64); // 0.001 ETH
    
    let gas_estimate = client.estimate_gas(to, value, None)
        .await
...
async fn test_transaction_sending() {
    ...
    let tx_hash = client.send_transaction(to, value, None)
        .await
        .expect("Failed to send transaction");
    
    // Wait for confirmation
    let receipt = client.wait_for_confirmation(tx_hash)
        .await
...
async fn test_multicall_support() {
    ...
    let multicall = client.create_multicall()
        .await
...
async fn test_network_switching() {
    ...
    client.switch_network(ChainConfig::base_mainnet())
        .await
    ...
    client.switch_network(ChainConfig::base_sepolia())
        .await
        .expect("Failed to switch network");
...
async fn test_nonce_management() {
    ...
    let _ = client.send_transaction(to, value, None).await.expect("Failed to send tx");
...
async fn test_gas_price_strategies() {
    ...
    // Legacy gas price
    let gas_price = client.get_gas_price().await.expect("Failed to get gas price");
    assert!(gas_price > U256::zero());
    
    // EIP-1559 gas pricing
    let (max_fee, priority_fee) = client.get_eip1559_gas_price()
        .await
...
async fn test_block_monitoring() {
    ...
    let mut block_stream = client.subscribe_blocks()
        .await
