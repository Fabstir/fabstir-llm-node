...
async fn test_node_registered_event_updates_cache() {
    ...
    monitor.handle_node_registered(node_addr, metadata.clone(), stake).await;
...
async fn test_node_updated_event_modifies_cache() {
    ...
    monitor.handle_node_updated(node_addr, new_metadata.clone()).await;
...
async fn test_concurrent_cache_access() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Register node
            monitor_clone.handle_node_registered(
                addr,
                format!("gpu:test{}", i),
                U256::from(i * 1000)
            ).await;
