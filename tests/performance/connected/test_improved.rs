...
async fn test_optimized_client() {
    ...
    if !failures.is_empty() {
        println!("First failure at vector {}: {}", failures[0].0, failures[0].1);
    }
...
async fn test_with_new_client_per_request() {
    ...
    for i in 0..count {
        ...
        match client.post("http://localhost:8080/api/v1/vectors")
            .json(&data)
            .send()
            .await {
            Ok(resp) if resp.status().is_success() => {
    ...
    println!("\n=== RESULTS ===");
    println!("Successfully inserted {} vectors (new client per request)", successful);
    println!("Total duration: {:?}", duration);
    println!("Throughput: {:.2} vectors/second", throughput);
}
...
async fn test_batch_api() {
    ...
    for batch_num in 0..(total_vectors / batch_size) {
        ...
        match client.post("http://localhost:8080/api/v1/vectors/batch")
            .json(&batch_data)
            .send()
            .await {
            Ok(resp) if resp.status().is_success() => {
                match resp.json::<serde_json::Value>().await {
                    Ok(result) => {
                        ...
                        println!("  Batch {} succeeded: {} vectors inserted", batch_num, successful);
                    },
                    Err(e) => {
                        println!("  Batch {} succeeded but couldn't parse response: {:?}", batch_num, e);
                        total_successful += batch_size;
                    }
                }
            },
            Ok(resp) => {
                eprintln!("  Batch {} failed with status: {}", batch_num, resp.status());
    ...
    println!("\n=== RESULTS ===");
    println!("Successfully inserted {} vectors using batch API", total_successful);
    println!("Total duration: {:?}", duration);
    println!("Throughput: {:.2} vectors/second", throughput);
}