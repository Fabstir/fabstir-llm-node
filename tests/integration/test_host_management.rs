use ethers::prelude::*;
use fabstir_llm_node::job_claim::{JobClaimer, ClaimError};
use fabstir_llm_node::job_assignment_types::{JobClaimConfig, AssignmentStatus};
use fabstir_llm_node::host::registry::{HostRegistry, HostInfo};
use fabstir_llm_node::host::selection::{HostSelector, PerformanceMetrics, JobRequirements};
...
async fn test_complete_host_management_workflow() {
    ...
    selector.update_performance_metrics(host1, PerformanceMetrics {
        jobs_completed: 100,
        success_rate: 0.98,
    ...
    selector.update_performance_metrics(host3, PerformanceMetrics {
        jobs_completed: 200,
        success_rate: 0.99,
    ...
    let best_host = selector.select_best_host(capable_hosts, &requirements).await.unwrap();
    ...
    claimer.assign_job_to_host(job_id, best_host, &registry).await.unwrap();
    ...
    claimer.reassign_job(job_id, new_host, &registry).await.unwrap();
...
async fn test_performance_with_100_hosts() {
    ...
    for i in 0..105 {
        ...
        selector.update_performance_metrics(host, PerformanceMetrics {
    ...
    for i in 0..50 {
        ...
        if let Some(host) = best {
            claimer.assign_job_to_host(&job_id, host, &registry).await.unwrap();
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let capable_addrs = registry_clone.get_available_hosts(&requirements.model_id).await;
            // Just pick first available host for concurrent test
            let best = capable_addrs.first().copied();
            
            if let Some(host) = best {
                claimer_clone.assign_job_to_host(&job_id, host, &registry_clone).await
            } else {
...
async fn test_load_balancing() {
    ...
    for i in 0..30 {
        let capable = registry.get_available_hosts(&requirements.model_id).await
            .into_iter()
            .filter_map(|addr| {
            ...
            .collect::<Vec<_>>();
        let best = selector.select_best_host(capable, &requirements).await.unwrap();
        ...
        *assignments.entry(best).or_insert(0) += 1;
        ...
        selector.update_performance_metrics(best, current_metrics).await;
    }
...
async fn test_failure_recovery() {
    ...
    claimer.assign_job_to_host(job_id, host1, &registry).await.unwrap();
