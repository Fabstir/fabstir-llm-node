...
async fn test_search_vector_db_for_similar_prompts() -> Result<()> {
    ...
    let cached_prompts = vec![
        ...
        ("Explain deep learning", "Deep learning is a type of machine learning..."),
        ("What is artificial intelligence?", "AI is the simulation of human intelligence..."),
        ("How does neural network work?", "Neural networks are computing systems..."),
    ];
    ...
    for (prompt, response) in &cached_prompts {
        ...
        vector_client.insert_vector(&prompt_hash, embedding, metadata).await?;
    }
    ...
    let metadata = first.get("metadata")
        .expect("Result should have metadata");
    
    // Check prompt in metadata
    let prompt = metadata.get("prompt")
        .and_then(|p| p.as_str())
        .expect("Metadata should have prompt field");
    
    assert!(
        prompt.contains("machine learning") || prompt.contains("AI") || prompt.contains("artificial intelligence"),
    ...
    assert!(score >= 0.5, "Score should be >= 0.5 for similar prompts, got: {}", score);
...
async fn test_retrieve_cached_results_from_s5() -> Result<()> {
    ...
    let cid = s5_client.put(&path, json_data.into_bytes(), Some(metadata)).await?;
    assert!(!cid.is_empty());
...
async fn test_measure_cache_hit_rates() -> Result<()> {
    ...
    let prompts = vec![
        ...
        ("Explain JavaScript", "JavaScript is a scripting language for web..."),
...
async fn test_complete_cache_flow() -> Result<()> {
    ...
    let similar_prompt = "What's the meaning of human life?;model=llama-3.2;temp=0.7;max_tokens=100";
...
async fn test_cache_expiration_and_cleanup() -> Result<()> {
    ...
    for i in 0..20 {
        ...
        let response = format!("Response with some data to take up space: {}", "x".repeat(50000));
