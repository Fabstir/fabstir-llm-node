use fabstir_llm_node::inference::{
    CleanupPolicy, DownloadProgress, ModelEventType, ModelManager, ModelRegistry, ModelRequest,
    ModelRequirements, ModelSource,
};
...
async fn test_model_verification() {
    ...
    let checksum = manager
        .calculate_checksum(&model_path)
        .await
        .expect("Failed to calculate checksum");

    let is_valid = manager
        .verify_model(&model_path, Some(&checksum))
        .await
        .expect("Failed to verify model");
...
async fn test_model_preloading() {
    ...
    let preload_handle = manager.preload_model(&model_path).await;
...
async fn test_model_conversion() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                e.to_string().contains("conversion") || e.to_string().contains("not supported")
            );
        }
    }
}
    ...
    for level in quant_levels {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    e.to_string().contains("quantization") || e.to_string().contains("not found")
                );
            }
        }
    }
...
async fn test_model_auto_download() {
    ...
    manager.set_preferred_sources(vec![ModelSource::HuggingFace {
    ...
    let model_path = manager
        .ensure_model_available(model_request)
        .await
...
async fn test_model_lifecycle_events() {
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e.event_type, ModelEventType::Registered)));
    assert!(events
        .iter()
        .any(|e| matches!(e.event_type, ModelEventType::Loaded)));
    assert!(events
        .iter()
        .any(|e| matches!(e.event_type, ModelEventType::Unloaded)));
}
...
async fn test_model_aliasing() {
    ...
    // Create aliases
    manager
        .add_model_alias("llama2-chat", &model_path)
        .await
        .expect("Failed to add alias");

    manager
        .add_model_alias("chat-model", &model_path)
        .await
        .expect("Failed to add alias");

    // Resolve aliases
    let resolved = manager
        .resolve_model_alias("llama2-chat")
        .await
