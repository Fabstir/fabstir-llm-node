...
use fabstir_llm_node::monitoring::{
    Dashboard, DashboardConfig, DashboardExport, DashboardManager, DataSource, GridPosition,
    Layout, Panel, Query, RefreshInterval, TimeRange, Visualization, Widget, WidgetType,
};
...
async fn test_data_queries() {
    ...
    // Add query
    let query = Query::new("prometheus", "rate(inference_requests_total[5m])")
...
async fn test_dashboard_variables() {
    ...
    manager
        .update_variable(&dash_id, "gpu_id", "2")
        .await
        .unwrap();
...
async fn test_real_time_updates() {
    ...
    // Receive updates
    let mut received = 0;
    while let Ok(Some(update)) = tokio::time::timeout(Duration::from_secs(2), updates.recv()).await
    {
        received += 1;
        if received >= 3 {
            break;
