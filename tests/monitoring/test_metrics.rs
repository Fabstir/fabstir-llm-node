...
async fn test_basic_counter_metrics() {
    ...
    let metric = collector.get_metric("inference_requests_total").await.unwrap();
...
async fn test_metric_rate_calculation() {
    ...
    collector.record_time_series_point("requests", 0.0).await.unwrap();
    tokio::time::sleep(Duration::from_millis(100)).await;
    ...
    for i in 0..10 {
        ...
        collector.record_time_series_point("requests", (i + 1) as f64 * 10.0).await.unwrap();
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
...
async fn test_metric_stream() {
    ...
    tokio::spawn(async move {
        for _i in 1..=5 {
            collector_clone.increment_counter_with_notification("test_stream_counter").await.unwrap();
...
async fn test_metric_garbage_collection() {
    ...
    assert!(metrics.iter().any(|m| m.last_updated.elapsed().as_secs() < 1));
}