...
async fn test_alert_grouping() {
    ...
    for i in 0..5 {
        ...
        manager.add_threshold_alert(rule).await.unwrap();
        manager.update_metric(&format!("service_{}_health", i), 0.0).await;
    }
...
async fn test_alert_silencing() {
    ...
    // Silence the alert
    manager.silence_alert(&alert_id, Duration::from_secs(300), "Testing silence").await.unwrap();
...
async fn test_alert_dependencies() {
    ...
    let mut child = ThresholdAlert::new(
        "api_errors",
        "api_error_rate",
        50.0,
        AlertLevel::Warning,
...
async fn test_alert_analytics() {
    ...
    for i in 0..10 {
        manager.update_metric("test_metric", if i % 2 == 0 { 60.0 } else { 40.0 }).await;
        manager.evaluate_alerts().await.unwrap();
...
async fn test_alert_recovery_actions() {
    ...
    rule.add_recovery_action(Box::new(|| Box::pin(async {
