...
fn test_error_display_model_not_registered() {
    let err = ModelValidationError::ModelNotRegistered("gpt-oss-120b.gguf".to_string());
    let display_msg = format!("{}", err);

    assert!(
        display_msg.contains("not registered"),
        "Should contain 'not registered'"
    );
    assert!(
        display_msg.contains("gpt-oss-120b.gguf"),
...
fn test_error_display_host_not_authorized() {
    ...
    assert!(
        display_msg.to_lowercase().contains("not authorized"),
    ...
    assert!(
        ...
        "Should contain full host address: actual='{}'",
        display_msg
    );
    assert!(
        display_msg.contains("0b75a2061e70e736924a30c0a327db7ab719402129f76f631adbd7b7a5a5bced"),
...
fn test_error_display_model_hash_mismatch() {
    ...
    assert!(
        display_msg.to_lowercase().contains("hash")
            || display_msg.to_lowercase().contains("mismatch"),
        "Should contain 'hash' or 'mismatch'"
    );
    assert!(
        display_msg.contains("/models/tampered.gguf") || display_msg.contains("tampered.gguf"),
...
fn test_error_debug_format() {
    let err = ModelValidationError::InvalidModelPath("/invalid/path/no_extension".to_string());
    let debug_msg = format!("{:?}", err);

    // Debug should contain the variant name and value
    assert!(
        debug_msg.contains("InvalidModelPath"),
        "Debug should contain variant name"
    );
    assert!(
        debug_msg.contains("/invalid/path/no_extension"),
...
fn test_validator_new_feature_enabled() {
    ...
    assert!(
        enabled,
        "Feature should be enabled when REQUIRE_MODEL_VALIDATION=true"
    );
...
fn test_error_context_preserved() {
    ...
    let model_id =
        H256::from_str("0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
            .unwrap();
    ...
    let expected =
        H256::from_str("0x1111111111111111111111111111111111111111111111111111111111111111")
            .unwrap();
    let actual =
        H256::from_str("0x2222222222222222222222222222222222222222222222222222222222222222")
            .unwrap();

    let err = ModelValidationError::ModelIdMismatch { expected, actual };

    match err {
        ModelValidationError::ModelIdMismatch {
            expected: e,
            actual: a,
        } => {
...
fn test_all_error_variants_exist() {
    ...
    let _err2 = ModelValidationError::HostNotAuthorized(Address::zero(), H256::zero());
