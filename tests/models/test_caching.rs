...
async fn test_cache_warmup() {
    ...
    let warmup_models = vec![
        ...
        (
            "bert-base",
            PathBuf::from("test_data/models/bert-base.onnx"),
        ),
    ];

    let strategy = WarmupStrategy::Parallel { max_concurrent: 2 };

    let warmup_result = cache.warmup_cache(warmup_models, strategy).await.unwrap();
...
async fn test_cache_event_stream() {
    ...
    while let Ok(Some(event)) =
        tokio::time::timeout(tokio::time::Duration::from_millis(100), event_stream.recv()).await
    {
        events.push(event);
    }
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e, CacheEvent::ModelLoaded { .. })));
    assert!(events
        .iter()
        .any(|e| matches!(e, CacheEvent::ModelAccessed { .. })));
    assert!(events
        .iter()
        .any(|e| matches!(e, CacheEvent::ModelEvicted { .. })));
}
