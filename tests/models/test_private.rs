...
async fn test_model_encryption() {
    ...
    let encrypted_path = manager.encrypt_model(
        &model_path,
        &owner,
        &encryption_config,
    ).await.unwrap();
    ...
    let decrypted_path = manager.decrypt_model(
        &encrypted_path,
        &owner,
    ).await.unwrap();
...
async fn test_access_token_generation() {
    ...
    let model_id = manager.create_private_model(
        PrivateModel::new("test-model", owner.clone()),
        &owner,
    ).await.unwrap();
...
async fn test_model_sharing() {
    ...
    // Create private model
    let model_id = manager.create_private_model(
        PrivateModel::new("shared-model", owner.clone()),
        &owner,
    ).await.unwrap();
    ...
    manager.share_model(&model_id, &owner, sharing).await.unwrap();
    ...
    let update_result = manager.update_model(&model_id, &collaborator, HashMap::new()).await;
...
async fn test_usage_tracking() {
    ...
    let model_id = manager.create_private_model(
        PrivateModel::new("metered-model", owner.clone()),
        &owner,
    ).await.unwrap();
    ...
    manager.set_usage_policy(&model_id, &owner, policy).await.unwrap();
...
async fn test_model_licensing() {
    ...
    let model_id = manager.create_private_model(
        PrivateModel::new("licensed-model", owner.clone()),
        &owner,
    ).await.unwrap();
    ...
    manager.set_license(&model_id, &owner, license.clone()).await.unwrap();
    ...
    let can_use = manager.check_license_compliance(&model_id, &user).await.unwrap();
    assert!(can_use);
}
...
async fn test_isolated_inference() {
    ...
    // Create private model
    let model_id = manager.create_private_model(
        PrivateModel::new("isolated-model", owner.clone()),
        &owner,
    ).await.unwrap();
    ...
    let session = manager.create_isolated_session(
        &model_id,
        &owner,
        isolation,
    ).await.unwrap();
...
async fn test_audit_logging() {
    ...
    // Create model
    let model_id = manager.create_private_model(
        PrivateModel::new("audited-model", owner.clone()),
        &owner,
    ).await.unwrap();
    
    // Perform various actions
    manager.update_model(&model_id, &owner, HashMap::from([
...
async fn test_private_model_registry() {
    ...
    let search_results = manager.search_private_models(
        "model",
        &owner,
        10,
    ).await.unwrap();
...
async fn test_rate_limiting() {
    ...
    let model_id = manager.create_private_model(
        PrivateModel::new("rate-limited", owner.clone()),
        &owner,
    ).await.unwrap();
    ...
    manager.set_rate_limits(&model_id, &owner, limits).await.unwrap();
    ...
    for i in 0..15 {
        ...
        let handle = tokio::spawn(async move {
            mgr.check_rate_limit(&mid, &usr).await
        });
        handles.push(handle);
    }
    
    let results: Vec<_> = futures::future::join_all(handles).await;
    let allowed = results.iter().filter(|r| r.as_ref().unwrap().is_ok()).count();
    let denied = results.iter().filter(|r| r.as_ref().unwrap().is_err()).count();
...
async fn test_model_deletion() {
    ...
    // Create model
    let model_id = manager.create_private_model(
        PrivateModel::new("temp-model", owner.clone()),
        &owner,
    ).await.unwrap();
    
    // Share with another user
    let other_user = ModelOwner::new("user456");
    manager.share_model(&model_id, &owner, SharingSettings {
    ...
    // Delete model
    manager.delete_private_model(&model_id, &owner).await.unwrap();
...
async fn test_model_export_restrictions() {
    ...
    let model_id = manager.create_private_model(
        PrivateModel::new("no-export-model", owner.clone()),
        &owner,
    ).await.unwrap();
    
    manager.set_export_policy(&model_id, &owner, ExportPolicy {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Export not allowed"));
...
async fn test_multi_tenant_isolation() {
    ...
    let model_a = manager.create_private_model(
        PrivateModel::new("tenant-a-model", tenant_a.clone()),
        &tenant_a,
    ).await.unwrap();
    
    let model_b = manager.create_private_model(
        PrivateModel::new("tenant-b-model", tenant_b.clone()),
        &tenant_b,
    ).await.unwrap();
    
    // Verify complete isolation
    assert!(manager.get_private_model(&model_a, &tenant_b).await.is_err());
    assert!(manager.get_private_model(&model_b, &tenant_a).await.is_err());
