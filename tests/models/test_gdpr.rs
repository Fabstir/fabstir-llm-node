...
async fn test_user_controlled_data_storage() {
    ...
    let encrypted = manager
        .encrypt_for_user(private_data.as_bytes(), &verifying_key)
        .await
        .unwrap();

    // Store encrypted data - node cannot decrypt
    let storage_proof = manager
    ...
    let retrieved = manager
        .retrieve_encrypted_data(&storage_proof.storage_id)
        .await
        .unwrap();
    let decrypted = manager
        .decrypt_with_user_key(&retrieved, &signing_key)
        .await
        .unwrap();
    ...
    assert!(manager
        .node_can_read_data(&storage_proof.storage_id)
        .await
        .is_err());
}
...
async fn test_decentralized_consent_management() {
    ...
    let tx_hash = manager
        .broadcast_consent_to_chain(signed_consent.clone())
        .await
        .unwrap();
    ...
    let verified = manager
        .verify_consent_on_chain(&verifying_key, "inference")
        .await
        .unwrap();
...
async fn test_p2p_right_to_erasure() {
    ...
    for i in 0..3 {
        ...
        let encrypted = manager
            .encrypt_for_user(data.as_bytes(), &verifying_key)
            .await
            .unwrap();
        manager
            .store_encrypted_data(&user_id, encrypted, RegionalPreference::Any)
            .await
            .unwrap();
    }
    ...
    let signed_request = manager
        .sign_deletion_request(deletion_request, &signing_key)
        .await
        .unwrap();

    // Broadcast deletion request to P2P network
    let deletion_proof = manager
        .broadcast_deletion_to_p2p(signed_request)
        .await
        .unwrap();
    ...
    let zk_proof = manager
        .generate_deletion_proof(&deletion_proof)
        .await
        .unwrap();
...
async fn test_user_controlled_data_portability() {
    ...
    let export_request = SignedRequest::new("EXPORT_ALL_MY_DATA", &signing_key);

    // Nodes return encrypted data that only user can decrypt
    let portable_package = manager
        .collect_user_data_p2p(&verifying_key, export_request)
        .await
        .unwrap();
    ...
    let decrypted_export = manager
        .decrypt_portable_package(portable_package, &signing_key)
        .await
        .unwrap();
    ...
    assert!(parsed["_metadata"]["decentralized_export"]
        .as_bool()
        .unwrap());
}
...
async fn test_regional_preference_routing() {
    ...
    let available_nodes = manager
        .discover_regional_nodes(preference.clone())
        .await
        .unwrap();
    ...
    let attestation = manager
        .verify_node_location(&selected_node.node_id, &selected_node.location_proof)
        .await
        .unwrap();
    ...
    let us_nodes = manager
        .discover_regional_nodes(RegionalPreference::US)
        .await
        .unwrap();
...
async fn test_zero_knowledge_compliance_proofs() {
    ...
    let compliance_proof = manager
        .generate_compliance_proof(ZkComplianceProof {
            proves: vec![
...
async fn test_consent_withdrawal_via_smart_contract() {
    ...
    let _consent_tx = manager
        .grant_consent_on_chain(consent, &signing_key)
        .await
        .unwrap();

    // User withdraws specific consent via smart contract
    let withdrawal = SignedRequest::new("WITHDRAW_CONSENT:analytics,model_training", &signing_key);

    let withdrawal_tx = manager
        .withdraw_consent_on_chain(withdrawal, &verifying_key)
        .await
        .unwrap();
...
async fn test_privacy_preserving_audit() {
    ...
    for (action, timestamp) in actions {
        let proof = manager
            .generate_audit_proof(AuditProof {
    ...
    let audit_verification = manager.verify_audit_trail(audit_proofs).await.unwrap();
...
async fn test_homomorphic_analytics_on_encrypted_data() {
    ...
    let users: Vec<_> = (0..10).map(|_| generate_user_keys()).collect();
    ...
    for (_signing_key, verifying_key) in &users {
        ...
        let encrypted = manager
            .homomorphic_encrypt(usage_data, verifying_key)
            .await
            .unwrap();

        manager
            .store_encrypted_metric(verifying_key, "tokens_used", encrypted)
            .await
            .unwrap();
    }

    // Compute aggregate statistics on encrypted data
    let total_usage = manager.compute_encrypted_sum("tokens_used").await.unwrap();
...
async fn test_federated_deletion_verification() {
    ...
    for node in &storage_nodes {
        let proof = manager
            .store_to_specific_node(&verifying_key, b"distributed data", node)
            .await
            .unwrap();
    ...
    let deletion = SignedRequest::new("DELETE_ALL", &signing_key);

    let deletion_result = manager
        .federated_delete(&verifying_key, deletion, storage_nodes.clone())
        .await
        .unwrap();
...
async fn test_user_controlled_anonymization() {
    ...
    let anonymized = manager
        .anonymize_with_user_rules(original, anon_prefs, &verifying_key)
        .await
        .unwrap();

    assert_eq!(
        anonymized.text,
        "My name is [NAME] and I live in [CITY], email: [EMAIL]"
    );
...
async fn test_decentralized_compliance_attestation() {
    ...
    let network_compliance = manager.aggregate_attestations(attestations).await.unwrap();
    ...
    let public_verification = manager
        .verify_network_compliance(&network_compliance)
        .await
        .unwrap();
