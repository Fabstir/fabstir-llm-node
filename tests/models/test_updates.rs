...
async fn test_version_comparison() {
    ...
    let test_cases = vec![
        (create_test_version(1, 0, 0), create_test_version(2, 0, 0), VersionComparison::Major),
        (create_test_version(1, 0, 0), create_test_version(1, 1, 0), VersionComparison::Minor),
        (create_test_version(1, 0, 0), create_test_version(1, 0, 1), VersionComparison::Patch),
        (create_test_version(1, 0, 0), create_test_version(1, 0, 0), VersionComparison::Equal),
        (create_test_version(2, 0, 0), create_test_version(1, 0, 0), VersionComparison::Downgrade),
    ];
...
async fn test_rollback_update() {
    ...
    assert_eq!(rollback_result.restored_version, create_test_version(1, 0, 0));
...
async fn test_batch_updates() {
    ...
    let batch_result = updater
        .check_batch_updates(models_to_update)
        .await
        .unwrap();
    ...
    if batch_result.updates_available > 0 {
        ...
        assert_eq!(update_result.successful_updates + update_result.failed_updates, 
...
async fn test_update_verification() {
    ...
    let verification_result = updater
        .verify_update(&model_path, &metadata)
        .await;
...
async fn test_migration_planning() {
    ...
    assert!(!migration_plan.breaking_changes.is_empty() || 
            migration_plan.breaking_changes.is_empty());
...
async fn test_update_cleanup() {
    ...
    let cleanup_result = updater
        .cleanup_old_versions("test_model", 2)
        .await
        .unwrap();
...
async fn test_update_failure_recovery() {
    ...
    let recovery_info = updater
        .get_recovery_info(model_id)
        .await
        .unwrap();
    
    assert!(recovery_info.can_recover);
    assert_eq!(recovery_info.last_stable_version, create_test_version(1, 0, 0));
}