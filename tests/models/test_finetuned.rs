...
use fabstir_llm_node::models::{
    ...
    ModelMerger, MergeStrategy, FineTuneValidator, FineTuneValidationResult as ValidationResult,
    FineTuneValidationLevel as ValidationLevel, InferenceSession, GenerationConfig,
    GenerationResponse,
};
...
async fn test_finetuned_model_registration() {
    ...
    let metadata = FineTuneMetadata {
        ...
        tags: vec!["medical".to_string(), "healthcare".to_string(), "qa".to_string()],
...
async fn test_adapter_loading() {
    ...
    std::fs::write(adapter_path.join("adapter_config.json"), r#"{"r": 8, "alpha": 16, "dropout": 0.1, "target_modules": ["q_proj", "v_proj"]}"#).unwrap();
...
async fn test_base_model_compatibility() {
    ...
    let compatible = manager.check_base_compatibility(
        "llama2-7b",
        "llama2-7b-base"
    ).await.unwrap();
    assert!(compatible);
    ...
    let incompatible = manager.check_base_compatibility(
        "llama2-7b",
        "gpt2-medium"
    ).await.unwrap();
    assert!(!incompatible);
...
async fn test_adapter_merging() {
    ...
    let merged_path = merger.merge_with_base(
...
async fn test_adapter_hot_swapping() {
    ...
    let medical_id = manager.register_finetuned(
        FineTuneMetadata {
            base_model: "llama2-7b".to_string(),
    ...
    let legal_id = manager.register_finetuned(
        FineTuneMetadata {
            base_model: "llama2-7b".to_string(),
    ...
    let medical_response = base_session.generate("Diagnose symptoms", Default::default()).await.unwrap();
    
    base_session.apply_adapter(&legal_id).await.unwrap();
    let legal_response = base_session.generate("Legal precedent for", Default::default()).await.unwrap();
...
async fn test_finetuned_model_validation() {
    ...
    let validation_result = validator.validate_finetuned(
        &model_id,
        &manager,
...
async fn test_adapter_caching() {
    ...
    // Register model
    let model_id = manager.register_finetuned(Default::default()).await.unwrap();
...
async fn test_finetuned_model_export() {
    ...
    let model_id = manager.register_finetuned(
        FineTuneMetadata {
            base_model: "llama2-7b".to_string(),
            fine_tune_type: FineTuneType::LoRA,
    ...
    manager.export_finetuned(&model_id, &export_path).await.unwrap();
...
async fn test_finetuned_capabilities() {
    ...
    let mut metadata = FineTuneMetadata::default();
    metadata.base_model = "llama2-7b".to_string();
    metadata.tags = vec!["medical".to_string(), "diagnosis".to_string(), "treatment".to_string()];
    ...
    let capabilities = FineTuneCapabilities {
        ...
        tasks: vec!["qa".to_string(), "diagnosis".to_string(), "treatment_planning".to_string()],
    ...
    manager.set_capabilities(&model_id, capabilities.clone()).await.unwrap();
