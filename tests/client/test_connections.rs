use fabstir_llm_node::api::{ApiServer, ApiConfig, ConnectionPool, ConnectionStats};
...
async fn test_connection_pool_creation() {
    ...
    let pool = ConnectionPool::new(pool_config).await.expect("Failed to create pool");
...
async fn test_connection_reuse() {
    ...
    let unique_connections = connection_ids.iter().collect::<std::collections::HashSet<_>>().len();
    assert!(unique_connections < 3, "Too many unique connections: {}", unique_connections);
}
...
async fn test_connection_limits() {
    ...
    let reject_count = results.iter().filter(|(_, status)| *status == 503 || *status == 0).count();
...
async fn test_idle_connection_cleanup() {
    ...
    // Make a request
    let _ = client.get(&url).send().await.expect("Failed to send request");
...
async fn test_graceful_connection_shutdown() {
    ...
    let long_request = tokio::spawn(async move {
        client_clone.post(&url_clone).json(&request).send().await
    });
