...
async fn test_enhanced_s5_health_check() -> Result<()> {
    ...
    let mut retries = 5;
    while retries > 0 {
        match config.client.get(config.api_url("/health")).send().await {
...
async fn test_enhanced_s5_basic_put_get() -> Result<()> {
    ...
    // GET file
    let response = config.client.get(config.api_url(test_path)).send().await?;
...
async fn test_enhanced_s5_path_structure() -> Result<()> {
    ...
    for (path, description) in test_cases {
        ...
        let response = config.client.get(config.api_url(path)).send().await?;
...
async fn test_enhanced_s5_content_types() -> Result<()> {
    ...
    // Retrieve and verify
    let response = config.client.get(config.api_url(cbor_path)).send().await?;
...
async fn test_enhanced_s5_concurrent_operations() -> Result<()> {
    ...
    assert_eq!(
        listing.entries.len(),
        10,
        "Not all concurrent files were created"
    );
