use fabstir_llm_node::storage::{
    ModelStorage, ModelMetadata, ModelFormat, CompressionType,
    StorageError, S5Storage, S5Client, S5StorageConfig, S5Backend,
    ModelVersion, ModelStorageConfig
};
use chrono::Utc;
...
mod tests {
    ...
    async fn test_store_model_basic() {
        ...
        let version = storage.store_model(
            &metadata.model_id,
            model_data.clone(),
            metadata.clone()
        ).await.unwrap();
    ...
    async fn test_retrieve_model() {
        ...
        let (retrieved_data, retrieved_metadata) = storage.get_model(&metadata.model_id).await.unwrap();
        ...
        let (version_data, version_metadata) = storage.get_model_version(
            &metadata.model_id,
            &version.version_id
        ).await.unwrap();
    ...
    async fn test_model_versioning() {
        ...
        metadata.metadata.insert("version_note".to_string(), "Improved accuracy".to_string());
        let v2 = storage.store_model(
            &metadata.model_id,
            v2_data.clone(),
            metadata.clone()
        ).await.unwrap();
        ...
        metadata.metadata.insert("version_note".to_string(), "Bug fixes".to_string());
        let v3 = storage.store_model(
            &metadata.model_id,
            v3_data.clone(),
            metadata.clone()
        ).await.unwrap();
        ...
        // List versions
        let versions = storage.list_model_versions(&metadata.model_id).await.unwrap();
        assert_eq!(versions.len(), 3);
        
        // Verify latest flag
        assert!(!versions.iter().find(|v| v.version_id == v1.version_id).unwrap().is_latest);
        assert!(!versions.iter().find(|v| v.version_id == v2.version_id).unwrap().is_latest);
        assert!(versions.iter().find(|v| v.version_id == v3.version_id).unwrap().is_latest);
        ...
        let (v1_retrieved, _) = storage.get_model_version(&metadata.model_id, &v1.version_id).await.unwrap();
    ...
    async fn test_model_compression() {
        ...
        let model_data: Vec<u8> = (0..uncompressed_size)
            .map(|i| (i % 10) as u8)
            .collect();
    ...
    async fn test_large_model_chunking() {
        ...
        let model_data: Vec<u8> = (0..large_size)
            .map(|i| (i % 256) as u8)
            .collect();
        ...
        let chunk_info = storage.get_chunk_info(&metadata.model_id, &version.version_id).await.unwrap();
        ...
        assert_eq!(retrieved_data[retrieved_data.len()-1000..], model_data[model_data.len()-1000..]);
    }
    ...
    async fn test_model_metadata_search() {
        ...
        for (id, tags) in models {
            ...
            storage.store_model(id, vec![0u8; 100], metadata).await.unwrap();
        }
        ...
        // Search by format
        let gguf_models = storage.search_models_by_format(ModelFormat::GGUF).await.unwrap();
    ...
    async fn test_model_integrity_verification() {
        ...
        // Verify integrity
        let is_valid = storage.verify_model_integrity(&metadata.model_id).await.unwrap();
        assert!(is_valid);
