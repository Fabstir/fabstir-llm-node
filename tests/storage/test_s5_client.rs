use fabstir_llm_node::storage::{
    ...
    S5EntryType, S5Metadata, StorageError, S5ClientConfig
};
...
mod tests {
    ...
    async fn create_test_storage() -> Result<Box<dyn S5Storage>, StorageError> {
        let config = match std::env::var("TEST_S5_BACKEND").as_deref() {
            Ok("real") => {
                ...
                S5StorageConfig {
                    backend: S5Backend::Real { portal_url: portal_url.clone() },
                    api_key,
    ...
    async fn test_put_with_metadata() {
        ...
        let cid = storage.put_with_metadata(path, data.clone(), metadata.clone()).await.unwrap();
        assert!(!cid.is_empty());
        
        // Retrieve and verify metadata
        let retrieved_metadata = storage.get_metadata(path).await.unwrap();
        assert_eq!(retrieved_metadata.get("content-type"), metadata.get("content-type"));
    ...
    async fn test_list_directory() {
        ...
        let has_configs_dir = entries.iter()
    ...
    async fn test_list_with_pagination() {
        ...
        for i in 0..25 {
            ...
            storage.put(&path, format!("content {}", i).into_bytes()).await.unwrap();
        }
        
        // List with limit
        let page1 = storage.list_with_options("home/pagination", Some(10), None).await.unwrap();
        assert_eq!(page1.entries.len(), 10);
        assert!(page1.cursor.is_some());
        
        // Get next page
        let page2 = storage.list_with_options(
            "home/pagination", 
            Some(10), 
            page1.cursor
        ).await.unwrap();
    ...
    async fn test_large_file_handling() {
        ...
        assert_eq!(retrieved[retrieved.len()-1000..], large_data[large_data.len()-1000..]);
    }
    ...
    async fn test_special_characters_in_paths() {
        ...
        for path in test_paths {
            ...
            assert!(retrieved.is_ok(), "Failed to retrieve {}: {:?}", path, retrieved);
    ...
    async fn test_path_validation() {
        ...
        for path in invalid_paths {
            ...
            assert!(result.is_err(), "Path '{}' should have failed validation", path);
        }
    }
    ...
    mod mock_tests {
        ...
        async fn test_error_injection() {
            ...
            storage.inject_error(StorageError::NetworkError("Simulated timeout".into())).await;
