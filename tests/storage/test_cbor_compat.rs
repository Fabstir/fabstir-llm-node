...
mod tests {
    ...
    async fn test_compression_compatibility() {
        ...
        let large_data: Vec<u8> = (0..10000).map(|i| (i % 10) as u8).collect();
        ...
        let encoded = compat
            .encode_with_compression(&test_struct, CompressionType::Zstd)
            .unwrap();
        ...
        // Decode
        let decoded: TestStruct = compat
            .decode_compressed(&encoded, CompressionType::Zstd)
            .unwrap();
    ...
    async fn test_nft_metadata_compatibility() {
        ...
        assert_eq!(
            decoded.properties.get("verified"),
