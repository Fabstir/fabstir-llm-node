use fabstir_llm_node::storage::{
    ...
    EvictionPolicy, StorageError, S5Storage, S5Client,
    S5StorageConfig, S5Backend
};
...
mod tests {
    ...
    async fn test_cache_put_and_get() {
        ...
        // Cache result
        cache.put(&key, result.as_bytes().to_vec(), Some(params.clone())).await.unwrap();
        ...
        assert_eq!(entry.metadata.get("model"), Some(&"llama-3.2-1b".to_string()));
    }
    ...
    async fn test_cache_eviction_lru() {
        ...
        cache.put("entry-10", vec![0u8; 200_000], None).await.unwrap();
    ...
    async fn test_cache_metadata() {
        ...
        cache.put(key, data.clone(), Some(metadata.clone())).await.unwrap();
        
        let entry = cache.get(key).await.unwrap().unwrap();
        assert_eq!(entry.metadata.get("model"), metadata.get("model"));
        assert_eq!(entry.metadata.get("temperature"), metadata.get("temperature"));
        assert_eq!(entry.metadata.get("tokens_generated"), metadata.get("tokens_generated"));
    }
    ...
    async fn test_cache_clear() {
        ...
        for i in 0..5 {
            cache.put(&format!("entry-{}", i), vec![i as u8; 100], None).await.unwrap();
        }
