use fabstir_llm_node::results::{
    S5StorageClient, S5StorageConfig, StorageMetadata, StorageResult,
...
mod tests {
    ...
    async fn test_store_result_returns_cid() {
        ...
        assert_eq!(storage_result.path, "/fabstir-llm/results/job_12345/result.cbor");
    ...
    async fn test_retrieve_result_by_cid() {
        ...
        assert_eq!(retrieved_result.result.job_id, original_result.result.job_id);
        assert_eq!(retrieved_result.result.response, original_result.result.response);
    ...
    async fn test_store_with_custom_metadata() {
        ...
        let storage_result = client.store_with_metadata(&packaged_result, metadata).await.unwrap();
    ...
    async fn test_concurrent_storage_operations() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                client.store_result(&result).await
            });
            handles.push(handle);
        }
