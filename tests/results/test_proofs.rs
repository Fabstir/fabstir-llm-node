use fabstir_llm_node::results::{
    ProofGenerator, InferenceProof, ProofType, ProofGenerationConfig, VerifiableResult,
    InferenceResult, PackagedResult, ResultMetadata
};
...
mod tests {
    ...
    async fn test_create_verifiable_result() {
        ...
        assert_eq!(verifiable.packaged_result.result.job_id, packaged.result.job_id);
    ...
    async fn test_verify_invalid_proof_fails() {
        ...
        let is_valid = generator.verify_proof(&proof, &modified_result).await.unwrap();
    ...
    async fn test_concurrent_proof_generation() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                generator.generate_proof(&result).await
            });
            handles.push(handle);
        }
