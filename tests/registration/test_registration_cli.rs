...
async fn test_register_single_chain_cli() -> Result<()> {
    ...
    env::set_var("NODE_PRIVATE_KEY", "0xe7855c0ea54ccca55126d40f97d90868b2a73bad0363e92ccdec0c4fbd6c0ce2");
...
async fn test_status_command() -> Result<()> {
    ...
    assert!(output.contains("Status") || output.contains("status") || output.contains("Not registered"));
...
async fn test_update_registration() -> Result<()> {
    ...
    assert!(output.contains("Update") || output.contains("update") || output.contains("Would update"));
...
async fn test_invalid_chain_cli() -> Result<()> {
    ...
    match result {
        ...
        Ok(output) => assert!(output.contains("Invalid") || output.contains("not supported"),
                              "Expected error message about invalid chain, got: {}", output),
    }
