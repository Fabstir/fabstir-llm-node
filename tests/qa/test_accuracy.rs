...
mod tests {
    ...
    async fn test_ground_truth_verification() {
        ...
        let result = verifier.verify_against_ground_truth(
    ...
    async fn test_format_validation() {
        ...
        verifier.add_validation_rule("json_response", json_rule).await;
        
        // Test valid JSON
        let valid_response = r#"{"result": "success", "value": 42}"#;
        let result = verifier.validate_format("json_response", valid_response).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_valid);
        
        // Test invalid JSON
        let invalid_response = "This is not JSON";
        let result = verifier.validate_format("json_response", invalid_response).await;
        assert!(result.is_ok());
        assert!(!result.unwrap().is_valid);
    }
    ...
    async fn test_semantic_similarity() {
        ...
        for (response, min_similarity) in similar_responses {
            let similarity = verifier.calculate_semantic_similarity(
                reference,
                response
            ).await;
    ...
    async fn test_accuracy_metrics_aggregation() {
        ...
        for (i, (is_accurate, score)) in results.iter().enumerate() {
            verifier.record_verification_result(
                &format!("job-{}", i),
                *is_accurate,
                *score
            ).await.unwrap();
    ...
    async fn test_model_specific_accuracy() {
        ...
        for (model, results) in model_results {
            for (i, is_accurate) in results.iter().enumerate() {
                verifier.record_model_verification(
                    model,
                    &format!("job-{}-{}", model, i),
                    *is_accurate,
                    0.9
    ...
    async fn test_verification_queue() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                verifier_clone.queue_verification(
                    &format!("job-{}", i),
                    "prompt",
                    "response",
                    None
