...
mod tests {
    ...
    async fn test_record_response_time() {
        ...
        for time_ms in times {
            let result = tracker.record_response_time(
                "llama-3.2-1b",
                "inference",
                time_ms
            ).await;
    ...
    async fn test_calculate_percentiles() {
        ...
        for i in 0..1000 {
            ...
            tracker.record_response_time("model", "op", time_ms).await.unwrap();
        }
    ...
    async fn test_model_specific_metrics() {
        ...
        for (model, times) in models {
            for time_ms in times {
                tracker.record_response_time(model, "inference", time_ms).await.unwrap();
            }
        }
    ...
    async fn test_performance_alerts() {
        ...
        for _ in 0..90 {
            tracker.record_response_time("model", "op", 100).await.unwrap();
        }
        
        // Record some slow responses that push p99 over threshold
        for _ in 0..10 {
            tracker.record_response_time("model", "op", 600).await.unwrap();
        }
    ...
    async fn test_sliding_window() {
        ...
        for _ in 0..10 {
            tracker.record_response_time("model", "op", 1000).await.unwrap();
        }
        ...
        for _ in 0..10 {
            tracker.record_response_time("model", "op", 50).await.unwrap();
        }
    ...
    async fn test_time_series_data() {
        ...
        let time_series = tracker.get_time_series_data(
            Duration::from_secs(1),
            5
        ).await;
    ...
    async fn test_comparison_metrics() {
        ...
        for _ in 0..100 {
            tracker.record_response_time("model", "op", 80).await.unwrap();
        }
