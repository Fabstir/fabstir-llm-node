...
mod tests {
    ...
    async fn test_calculate_average_ratings() {
        ...
        assert_eq!(
            summary.category_averages[&RatingCategory::ResponseQuality],
            4.6
        );
    ...
    async fn test_reputation_impact() {
        ...
        for i in 0..10 {
            ...
            manager
                .submit_rating_for_host("host-123", rating)
                .await
                .unwrap();
        }
    ...
    async fn test_feedback_analysis() {
        ...
        let feedback_samples = vec![
            (
                "Excellent quality, very fast responses!",
                5,
                FeedbackType::Positive,
            ),
            ("Good overall but a bit expensive", 4, FeedbackType::Neutral),
            (
                "Responses were slow and sometimes inaccurate",
                2,
    ...
    async fn test_time_based_ratings() {
        ...
        let recent = manager
            .get_recent_ratings("llama-3.2-1b", Duration::hours(24))
            .await;
