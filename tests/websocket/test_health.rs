use fabstir_llm_node::api::websocket::{
    config::{ProductionConfig, ConfigManager},
    health::{HealthChecker, HealthStatus, ReadinessCheck, LivenessCheck},
};
...
async fn test_liveness_probe() {
    ...
    liveness.simulate_hang("websocket", Duration::from_secs(31)).await;
    assert!(!liveness.is_alive().await);
}
