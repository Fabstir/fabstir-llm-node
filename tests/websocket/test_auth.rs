use fabstir_llm_node::api::websocket::auth::{
    Authenticator, AuthConfig, AuthResult, AuthError,
    JobVerifier, SessionToken, Permission,
};
...
async fn test_authentication_failures() {
    ...
    // Tampered token
    let token = auth.create_session_token("session-1", 12345, vec![]).await.unwrap();
...
async fn test_permission_checks() {
    ...
    assert!(auth.check_permission(&token.token, Permission::Read).await.unwrap());
    
    // Check denied permission
    assert!(!auth.check_permission(&token.token, Permission::Write).await.unwrap());
    assert!(!auth.check_permission(&token.token, Permission::Admin).await.unwrap());
}
...
async fn test_signature_verification() {
    ...
    // Different message
    assert!(!auth.verify_signature("different", &signature).await.unwrap());
}
...
async fn test_multi_factor_authentication() {
    ...
    auth.verify_signature("session-1:12345", &signature).await.unwrap();
    
    // Create MFA token
    let token = auth.create_mfa_token(
        "session-1",
        12345,
        &signature,
    ).await.unwrap();
...
async fn test_role_based_access_control() {
    ...
    let user_token = auth.create_session_token(
        "user-session",
        12345,
        vec![Permission::Read]
    ).await.unwrap();
    
    let host_token = auth.create_session_token(
        "host-session",
        12345,
        vec![Permission::Read, Permission::Write, Permission::Execute]
    ).await.unwrap();
    
    let admin_token = auth.create_session_token(
        "admin-session",
        12345,
