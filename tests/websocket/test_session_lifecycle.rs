...
async fn test_concurrent_session_access() {
    ...
    let session_id = store_arc.write().await.create_session(SessionConfig::default()).await;
    ...
    let session = store_arc.read().await.get_session(&session_id).await.unwrap();
