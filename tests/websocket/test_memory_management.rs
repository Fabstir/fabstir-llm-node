...
use fabstir_llm_node::api::websocket::{
    memory_manager::{MemoryConfig, MemoryManager, MemoryStats, SessionPool},
...
async fn test_session_memory_limits() {
    ...
    for _ in 0..5 {
        manager
            .add_session_data(&session_id, large_data.clone())
            .await
            .unwrap();
    }
...
async fn test_memory_defragmentation() {
    ...
    for i in 0..20 {
        if i % 2 == 0 {
            manager
                .remove_session(&format!("session-{}", i))
                .await
                .unwrap();
        }
    }
...
async fn test_memory_cleanup_on_drop() {
...

    {
        let manager = MemoryManager::new(config);
        manager
            .add_session("temp-session".to_string())
            .await
            .unwrap();
