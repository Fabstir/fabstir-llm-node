use fabstir_llm_node::api::websocket::auth::{
    SignatureVerifier, SignatureConfig
};
...
async fn test_ed25519_signature_generation() {
    ...
    assert_eq!(signature.len(), 128, "Ed25519 signature should be 64 bytes (128 hex chars)");
...
async fn test_ed25519_signature_verification() {
    ...
    let is_valid = verifier.verify_signature(message, &signature).await.unwrap();
...
async fn test_ed25519_invalid_signature_rejection() {
    ...
    let is_valid = verifier.verify_signature("Modified message", &signature).await.unwrap();
    assert!(!is_valid, "Modified message should not verify");
    
    // Test with modified signature
    let mut bad_signature = signature.clone();
    bad_signature.replace_range(0..2, "FF");
    let is_valid = verifier.verify_signature(message, &bad_signature).await.unwrap();
...
async fn test_ed25519_different_keys_rejection() {
    ...
    let is_valid = verifier2.verify_signature(message, &signature).await.unwrap();
...
async fn test_signature_format_validation() {
    ...
    for invalid_sig in invalid_signatures {
        ...
        assert!(result.is_err() || !result.unwrap(), 
               "Invalid signature format should be rejected: {}", invalid_sig);
    }
}
...
async fn test_batch_signature_verification() {
    ...
    for i in 0..messages.len() {
        ...
        let is_valid = verifier.verify_signature(messages[i], &signatures[j]).await.unwrap();
...
async fn test_concurrent_signature_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let is_valid = verifier_clone.verify_signature(&message, &signature).await.unwrap();
