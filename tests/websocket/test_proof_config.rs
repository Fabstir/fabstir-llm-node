...
async fn test_proof_disabled_returns_none() -> Result<()> {
    ...
    let proof = manager
        .generate_proof_optional("model", "prompt", "output")
        .await?;
...
async fn test_response_handler_respects_proof_config() -> Result<()> {
    ...
    session_handler
        .handle_session_init("test-disabled", 123, vec![])
        .await?;

    // Create stream and get final token
    let mut stream = response_handler
        .create_response_stream("test-disabled", "Test", 0)
        .await?;
...
async fn test_proof_cache_size_configuration() -> Result<()> {
    ...
    let proof1_cached = manager
        .generate_proof("model", "prompt1", "output1")
        .await?;
    let proof2_cached = manager
        .generate_proof("model", "prompt2", "output2")
        .await?;
    assert_eq!(
        proof1.timestamp, proof1_cached.timestamp,
        "Proof1 should be cached"
    );
    assert_eq!(
        proof2.timestamp, proof2_cached.timestamp,
        "Proof2 should be cached"
    );

    // Generate many more proofs to trigger eviction
    for i in 3..10 {
        manager
            .generate_proof("model", &format!("prompt{}", i), &format!("output{}", i))
            .await?;
    }
    ...
    let proof1_new = manager
        .generate_proof("model", "prompt1", "output1")
        .await?;
    let proof2_new = manager
        .generate_proof("model", "prompt2", "output2")
        .await?;

    // At least one should have been regenerated (different timestamp)
    let was_evicted =
        proof1.timestamp != proof1_new.timestamp || proof2.timestamp != proof2_new.timestamp;
...
async fn test_proof_batch_configuration() -> Result<()> {
    ...
    for i in 0..6 {
        ...
        let handle = tokio::spawn(async move {
            m.generate_proof("model", &format!("prompt{}", i), "output")
                .await
        });
        handles.push(handle);
    }
