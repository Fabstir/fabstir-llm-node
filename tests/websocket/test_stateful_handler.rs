...
use fabstir_llm_node::api::websocket::{
    handler::{HandlerConfig, WebSocketHandler},
    message_types::{InferenceMessage, MessageType, SessionControl, WebSocketMessage},
...
async fn test_automatic_context_building() {
    ...
    let context = handler
        .build_context_for_session(&session_id, "What is its population?")
        .await
        .unwrap();
...
async fn test_concurrent_session_handling() {
    let store = SessionStore::new(SessionStoreConfig::default());
    let config = HandlerConfig::default();
    let handler = Arc::new(RwLock::new(WebSocketHandler::new(
