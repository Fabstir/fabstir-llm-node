...
use fabstir_llm_node::api::websocket::{
    integration::*, manager::SessionManager, session::*,
};
use std::sync::Arc;
...
async fn test_job_processor_session_support() -> Result<()> {
    ...
    assert_eq!(session.metadata.read().await.get("job_id"), Some(&job_id.to_string()));
    assert_eq!(session.state, SessionState::Active);
    
    // Process job stages with session
    integration.update_job_stage(&session, "preprocessing").await?;
    integration.update_job_stage(&session, "inference").await?;
    integration.update_job_stage(&session, "postprocessing").await?;
...
async fn test_session_handoff_between_workers() -> Result<()> {
    ...
    let worker_2_session = integration.get_worker_session("worker-2", &session.id).await?;
...
async fn test_load_balancing_integration() -> Result<()> {
    ...
    // Configure workers
    integration.configure_workers(vec!["worker-1", "worker-2", "worker-3"]).await?;
