...
async fn test_e2e_multi_turn_conversation() -> Result<()> {
    ...
    // First turn
    session
        .add_message_async("user", "My name is Alice")
        .await?;
...
async fn test_e2e_session_recovery() -> Result<()> {
    ...
    // Add some context
    session
        .add_message_async("user", "Remember the number 42")
        .await?;
...
async fn test_e2e_load_testing() -> Result<()> {
    ...
    for i in 0..50 {
        integration
            .process_session_request(&session, &format!("Request {}", i))
            .await?;
    }
...
async fn test_e2e_heartbeat_mechanism() -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        session.metadata.write().await.insert(
...
async fn test_e2e_metadata_sync() -> Result<()> {
    ...
    session
        ...
        .write()
        .await
        .insert("model".to_string(), "llama2".to_string());
    session
        .metadata
        .write()
        .await
