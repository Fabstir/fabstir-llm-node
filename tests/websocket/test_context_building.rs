...
async fn test_build_context_from_session() {
    ...
    let context = manager.build_context(&session, current_prompt).await.unwrap();
...
async fn test_sliding_window_context() {
    ...
    let context = manager.build_context(&session, "New question").await.unwrap();
...
async fn test_format_context_for_llm() {
    ...
    let formatted = manager.format_for_llm(&messages, "What is 3+3?").await.unwrap();
