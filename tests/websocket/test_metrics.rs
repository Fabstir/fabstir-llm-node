use fabstir_llm_node::api::websocket::metrics::{
    MetricType, MetricsExporter, PrometheusMetrics, WebSocketMetrics,
};
...
async fn test_prometheus_metrics_collection() {
    ...
    metrics
        .record_inference_time(Duration::from_millis(150))
        .await;
...
async fn test_metric_types() {
    ...
    // Test Counter
    let counter = metrics
        .get_metric(MetricType::Counter("websocket_sessions_total"))
        .await;
    assert_eq!(counter, 0);
    ...
    let gauge = metrics
        .get_metric(MetricType::Gauge("websocket_active_sessions"))
        .await;
    assert_eq!(gauge, 5);

    // Test Histogram
    metrics
        .record_inference_time(Duration::from_millis(100))
        .await;
    metrics
        .record_inference_time(Duration::from_millis(200))
        .await;
    let histogram = metrics
        .get_metric(MetricType::Histogram("websocket_inference_duration_ms"))
        .await;
...
async fn test_websocket_specific_metrics() {
    ...
    metrics
        .message_received("session-1", "response", 2048)
        .await;
...
async fn test_metrics_aggregation() {
    ...
    for i in 0..10 {
        ...
        metrics
            .record_inference_time(Duration::from_millis((50 + i * 10) as u64))
            .await;
    }
...
async fn test_custom_metrics_registration() {
    ...
    let value = metrics
        .get_metric(MetricType::Counter("llm_cache_hits"))
        .await;
    assert_eq!(value, 2);
}
...
async fn test_metrics_cleanup() {
    ...
    for i in 0..100 {
        metrics
            .record_session_created(&format!("session-{}", i))
            .await;
    }
    ...
    for i in 0..50 {
        metrics
            .record_session_closed(&format!("session-{}", i))
            .await;
    }
