...
async fn test_proof_determinism_by_type() -> Result<()> {
    ...
    let proof1 = manager.generate_proof("model", "same_input", "same_output").await?;
    let proof2 = manager.generate_proof("model", "same_input", "same_output").await?;
...
async fn test_proof_size_by_type() -> Result<()> {
    ...
    let simple_proof = manager_simple.generate_proof("model", "input", "output").await?;
    let ezkl_proof = manager_ezkl.generate_proof("model", "input", "output").await?;
...
async fn test_proof_type_performance() -> Result<()> {
    ...
    manager_simple.generate_proof("model", "input", "output").await?;
    let simple_time = start.elapsed();
    
    // Measure EZKL proof generation time
    let start = Instant::now();
    manager_ezkl.generate_proof("model", "input", "output").await?;
    ...
    println!("Simple proof time: {:?}, EZKL proof time: {:?}", simple_time, ezkl_time);
...
async fn test_mixed_proof_types_in_session() -> Result<()> {
    ...
    let proof1 = manager1.generate_proof("model", "prompt1", "output1").await?;
    ...
    let proof2 = manager2.generate_proof("model", "prompt2", "output2").await?;
