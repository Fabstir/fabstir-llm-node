...
use fabstir_llm_node::job_assignment_types::{JobClaimConfig, AssignmentRecord, AssignmentStatus};
...
async fn create_mock_selector() -> Arc<HostSelector> {
    ...
    selector.update_performance_metrics(host2, PerformanceMetrics {
        jobs_completed: 50,
        success_rate: 0.95,
...
async fn test_assign_job_to_specific_host() {
    ...
    let result = claimer.assign_job_to_host(job_id, host_address, &registry).await;
    assert!(result.is_ok());
...
async fn test_batch_assign_multiple_jobs() {
    ...
    let results = claimer.batch_assign_jobs(assignments, &registry).await.unwrap();
...
async fn test_reassign_job_to_new_host() {
    ...
    claimer.assign_job_to_host(job_id, original_host, &registry).await.unwrap();
...
async fn test_auto_assign_picks_best_host() {
    ...
    let result = claimer.auto_assign_job(job_id, &registry, &selector, &requirements).await;
    assert!(result.is_ok());
...
async fn test_assignment_failure_handling() {
    ...
    let result = claimer.assign_job_to_host(job_id, invalid_host, &registry).await;
...
async fn test_priority_jobs_assigned_first() {
    ...
    // Process assignments
    let processed = claimer.process_priority_assignments(host, &registry, 3).await;
...
async fn test_get_host_assignments() {
    ...
    claimer.assign_job_to_host("job-001", host1, &registry).await.unwrap();
    claimer.assign_job_to_host("job-002", host1, &registry).await.unwrap();
    claimer.assign_job_to_host("job-003", host2, &registry).await.unwrap();
