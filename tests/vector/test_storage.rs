use fabstir_llm_node::vector::{
    ...
    VectorId, VectorEntry, StorageMetadata, VectorStorageError as StorageError,
    MigrationConfig, MigrationStatus, MigrationStatusType, IndexType, StorageStats,
    S5Storage, S5StorageConfig, S5Backend, S5Client
};
...
mod tests {
    ...
    async fn test_batch_vector_storage() {
        ...
        let vectors: Vec<VectorEntry> = (0..50)
            .map(|i| {
                ...
                entry.metadata.insert("batch_index".to_string(), i.to_string());
                entry
            })
            .collect();
    ...
    async fn test_storage_paths() {
        ...
        for category in categories {
            for i in 0..3 {
                ...
                entry.metadata.insert("category".to_string(), category.to_string());
                
                let custom_path = format!("home/vectors/{}/{}", category, id);
                storage.store_vector_with_path(&entry, &custom_path).await.unwrap();
            }
        }
    ...
    async fn test_metadata_indexing() {
        ...
        for i in 0..20 {
            ...
            entry.metadata.insert("score".to_string(), (i * 10).to_string());
            entry.metadata.insert("category".to_string(), 
                if i % 2 == 0 { "even" } else { "odd" }.to_string());
            entry.metadata.insert("tags".to_string(), 
    ...
    async fn test_storage_statistics() {
        ...
        assert_eq!(updated_stats.total_vectors, initial_stats.total_vectors + 10);
    ...
    async fn test_vector_deletion() {
        ...
        // Batch delete
        storage.delete_batch(&["del_vec_1", "del_vec_3"]).await.unwrap();
    ...
    async fn test_storage_backup_restore() {
        ...
        // Clear storage
        for i in 0..5 {
            storage.delete_vector(&format!("backup_vec_{}", i)).await.unwrap();
        }
        
        // Restore from backup
        let restore_result = storage.restore_from_backup(&backup_result.backup_id).await.unwrap();
        ...
        // Verify restoration
        for i in 0..5 {
            assert!(storage.vector_exists(&format!("backup_vec_{}", i)).await.unwrap());
        }
    }
    ...
    async fn test_compression_effectiveness() {
        ...
        let info = storage.get_compression_info(&result.vector_id).await.unwrap();
