...
mod tests {
    ...
    async fn test_get_vector_by_id() {
        ...
        assert_eq!(
            retrieved.metadata.get("model"),
            vector.metadata.get("model")
        );
    }
    ...
    async fn test_search_with_options() {
        ...
        let results = client
            .search_with_options(query_vector, options)
            .await
            .unwrap();

        // All results should be from category A
        for result in &results {
            assert_eq!(result.metadata.get("category"), Some(&"A".to_string()));
        }
    }
    ...
    async fn test_complex_metadata_filter() {
        ...
        for i in 0..20 {
            ...
            vector.metadata.insert(
                "tags".to_string(),
                serde_json::to_string(&vec_tags).unwrap(),
            );
            vector
                .metadata
                .insert("duration_seconds".to_string(), ((i + 1) * 100).to_string());
            vector
                .metadata
                .insert("views".to_string(), ((i + 1) * 1000).to_string());
        ...
        let filter = HashMap::from([
            (
                "tags".to_string(),
                FilterValue::Array(vec!["ai".to_string(), "ml".to_string()]),
            ),
            (
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            let exists = client
                .vector_exists(&format!("concurrent_vec_{}", i))
                .await
                .unwrap();
            assert!(exists);
        }
    ...
    async fn test_streaming_updates() {
        ...
        let update =
            tokio::time::timeout(std::time::Duration::from_secs(2), update_stream.next()).await;
