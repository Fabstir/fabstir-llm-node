...
use fabstir_llm_node::vector::{
    CacheEntry, CacheError, CacheEvictionPolicy, CacheHit, CacheStats, EmbeddingConfig,
    EmbeddingGenerator, EmbeddingModel, SemanticCache, SemanticCacheConfig, SimilarityThreshold,
    VectorBackend, VectorDBClient, VectorDBConfig,
};
...
mod tests {
    ...
    async fn create_test_cache() -> Result<SemanticCache, CacheError> {
        ...
        Ok(SemanticCache::new(cache_config, embedding_generator, vector_client).await?)
    }
    ...
    async fn test_cache_with_metadata() {
        ...
        // Store with metadata
        cache
            .store(prompt, response, Some(metadata.clone()))
            .await
            .unwrap();
    ...
    async fn test_cache_ttl_expiration() {
        ...
        let embedding_generator = EmbeddingGenerator::new(EmbeddingConfig::default())
            .await
            .unwrap();
        let vector_client = VectorDBClient::new(VectorDBConfig::default())
            .await
            .unwrap();

        let cache = SemanticCache::new(cache_config, embedding_generator, vector_client)
            .await
            .unwrap();
    ...
    async fn test_namespace_isolation() {
        ...
        let embedding_generator1 = EmbeddingGenerator::new(embedding_config.clone())
            .await
            .unwrap();
        ...
        let cache1 = SemanticCache::new(config1, embedding_generator1, vector_client1)
            .await
            .unwrap();

        let cache2 = SemanticCache::new(config2, embedding_generator2, vector_client2)
            .await
            .unwrap();
        ...
        cache1
            .store(prompt, "Response from namespace 1", None)
            .await
            .unwrap();
        cache2
            .store(prompt, "Response from namespace 2", None)
            .await
            .unwrap();
    ...
    async fn test_cache_performance_metrics() {
        ...
        for (i, prompt) in test_prompts.iter().enumerate() {
            cache
                .store(prompt, &format!("Response {}", i), None)
                .await
                .unwrap();
        }
