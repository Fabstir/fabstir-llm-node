...
mod tests {
    ...
    async fn test_calculate_token_price() {
        ...
        for (tokens, expected_price) in test_cases {
            let price = manager
                .calculate_token_price("llama-3.2-1b-instruct", tokens)
                .await;
            assert!(price.is_ok());
    ...
    async fn test_calculate_time_based_price() {
        ...
        let price = manager
            .calculate_time_price("llama-3.2-1b-instruct", duration_minutes)
            .await;
    ...
    async fn test_dynamic_pricing() {
        ...
        let price = manager
            .calculate_token_price_with_demand("llama-3.2-1b-instruct", 1_000)
            .await;
        assert!(price.is_ok());
    ...
    async fn test_multi_currency_support() {
        ...
        let usdc_price = manager
            .get_pricing_by_currency("llama-3.2-1b-instruct", Currency::USDC)
            .await;
        assert!(usdc_price.is_some());

        let fab_price = manager
            .get_pricing_by_currency("mistral-7b", Currency::FAB)
            .await;
    ...
    async fn test_pricing_history() {
        ...
        for i in 1..=3 {
            ...
            manager
                .update_base_price("llama-3.2-1b-instruct", 0.000001 * (i as f64))
                .await
                .unwrap();
        }

        let history = manager
            .get_pricing_history("llama-3.2-1b-instruct", 10)
            .await;
    ...
    async fn test_promotional_pricing() {
        ...
        let price = manager
            .calculate_token_price("llama-3.2-1b-instruct", 1_000)
            .await;
        assert!(price.is_ok());
