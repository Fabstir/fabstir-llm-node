...
mod tests {
    ...
    fn create_test_schedule() -> AvailabilitySchedule {
        AvailabilitySchedule {
            ...
            weekly_schedule: vec![
                ...
                (Weekday::Tue, NaiveTime::from_hms_opt(9, 0, 0).unwrap(), NaiveTime::from_hms_opt(17, 0, 0).unwrap()),
                (Weekday::Wed, NaiveTime::from_hms_opt(9, 0, 0).unwrap(), NaiveTime::from_hms_opt(17, 0, 0).unwrap()),
                (Weekday::Thu, NaiveTime::from_hms_opt(9, 0, 0).unwrap(), NaiveTime::from_hms_opt(17, 0, 0).unwrap()),
                (Weekday::Fri, NaiveTime::from_hms_opt(9, 0, 0).unwrap(), NaiveTime::from_hms_opt(17, 0, 0).unwrap()),
            ],
    ...
    async fn test_set_availability_schedule() {
        ...
        assert!(matches!(current_status, AvailabilityStatus::Available | AvailabilityStatus::Unavailable));
    }
    ...
    async fn test_check_availability_at_time() {
        ...
        let monday_10am = DateTime::parse_from_rfc3339("2024-01-15T10:00:00Z").unwrap().with_timezone(&Utc);
        ...
        let monday_8pm = DateTime::parse_from_rfc3339("2024-01-15T20:00:00Z").unwrap().with_timezone(&Utc);
        ...
        let saturday = DateTime::parse_from_rfc3339("2024-01-13T12:00:00Z").unwrap().with_timezone(&Utc);
    ...
    async fn test_capacity_management() {
        ...
        for i in 0..8 {
            let result = manager.allocate_capacity("llama-3.2-1b", &format!("job-{}", i)).await;
            assert!(result.is_ok());
        }
        ...
        // Release capacity
        manager.release_capacity("llama-3.2-1b", "job-0").await.unwrap();
    ...
    async fn test_availability_notifications() {
        ...
        // Trigger a change
        manager.set_status(AvailabilityStatus::Maintenance).await.unwrap();
        
        let notification = receiver.recv().await;
        assert!(notification.is_ok());
        assert_eq!(notification.unwrap().new_status, AvailabilityStatus::Maintenance);
    }
    ...
    async fn test_availability_exceptions() {
        ...
        schedule.exceptions.push((
            DateTime::parse_from_rfc3339("2024-12-25T00:00:00Z").unwrap().with_timezone(&Utc),
            DateTime::parse_from_rfc3339("2024-12-25T23:59:59Z").unwrap().with_timezone(&Utc),
        ...
        let christmas = DateTime::parse_from_rfc3339("2024-12-25T12:00:00Z").unwrap().with_timezone(&Utc);
    ...
    async fn test_graceful_shutdown() {
        ...
        manager.set_capacity_config(CapacityConfig {
            max_concurrent_jobs: 10,
            reserved_capacity: 0,
            max_queue_size: 0,
            models: vec!["llama-3.2-1b".to_string()],
        ...
        manager.allocate_capacity("llama-3.2-1b", "job-1").await.unwrap();
        manager.allocate_capacity("llama-3.2-1b", "job-2").await.unwrap();
        
        // Initiate graceful shutdown
        let shutdown_handle = manager.initiate_graceful_shutdown(Duration::minutes(5)).await;
    ...
    async fn test_availability_forecast() {
        ...
        let has_maintenance = forecast.iter().any(|period| {
            matches!(period.status, AvailabilityStatus::Maintenance)
        });
