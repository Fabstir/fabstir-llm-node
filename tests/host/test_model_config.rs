...
mod tests {
    ...
    async fn test_update_model_parameters() {
        ...
        let result = manager.update_model_parameters("llama-3.2-1b-instruct", new_params).await;
        assert!(result.is_ok());
    ...
    async fn test_enable_disable_model() {
        ...
        // Disable model
        let result = manager.set_model_status("llama-3.2-1b-instruct", ModelStatus::Disabled).await;
        assert!(result.is_ok());
        ...
        // Enable model
        let result = manager.set_model_status("llama-3.2-1b-instruct", ModelStatus::Enabled).await;
        assert!(result.is_ok());
    ...
    async fn test_model_metadata_update() {
        ...
        let new_metadata = ModelMetadata {
            ...
            tags: vec!["updated".to_string(), "fast".to_string(), "efficient".to_string()],
            capabilities: vec!["chat".to_string(), "completion".to_string(), "reasoning".to_string()],
            languages: vec!["en".to_string(), "es".to_string(), "fr".to_string()],
            version: "3.2.1".to_string(),
        };
        
        let result = manager.update_model_metadata("llama-3.2-1b-instruct", new_metadata.clone()).await;
        assert!(result.is_ok());
