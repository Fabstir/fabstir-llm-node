...
mod tests {
    use super::*;
    use fee_distributor::{FeeDistributor, ContractClient};
    use std::sync::Arc;
    use tokio::sync::RwLock;
    ...
    async fn test_fee_allocation_with_referrer() {
        ...
        // 40% marketplace
        assert_eq!(allocation.marketplace_share, U256::from(40_000_000_000_000_000u64));
        
        // 30% network
        assert_eq!(allocation.network_share, U256::from(30_000_000_000_000_000u64));
        
        // 20% referrer
        assert_eq!(allocation.referrer_share, U256::from(20_000_000_000_000_000u64));
        
        // 10% burn
        assert_eq!(allocation.burn_amount, U256::from(10_000_000_000_000_000u64));
    ...
    async fn test_fee_allocation_without_referrer() {
        ...
        let remaining = allocation.marketplace_share + 
                       allocation.network_share + 
                       allocation.burn_amount;
    ...
    async fn test_recipient_registration() {
        ...
        // Register recipients
        distributor.register_recipient(
    ...
    async fn test_pending_fee_accumulation() {
        ...
        let pending = distributor.get_pending_fees(marketplace_addr).await.unwrap();
    ...
    async fn test_fee_distribution() {
        ...
        assert!(distributed.iter().any(|(addr, _)| *addr == marketplace_addr));
    ...
    async fn test_fee_statistics() {
        ...
        assert_eq!(stats.total_fees_collected, U256::from(100_000_000_000_000_000u64));
        assert!(stats.fees_by_role.contains_key(&RecipientRole::MarketplaceOperator));
        assert!(stats.fees_by_role.contains_key(&RecipientRole::NetworkMaintainer));
    }
}