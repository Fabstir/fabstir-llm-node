...
mod tests {
    ...
    use withdrawal_manager::{ContractClient, WithdrawalManager};
    ...
    async fn test_request_withdrawal_success() {
        ...
        manager
            .update_available_balance(token, U256::from(1_000_000_000_000_000_000u64))
            .await
            .unwrap();
    ...
    async fn test_minimum_withdrawal_enforcement() {
        ...
        let result = manager
            .request_withdrawal(amount, Address::zero(), Address::random())
            .await;
    ...
    async fn test_cooldown_period_enforcement() {
        ...
        // Update balance
        manager
            .update_available_balance(token, U256::from(10_000_000_000_000_000_000u64))
            .await
            .unwrap();
        ...
        // Execute it
        manager
            .execute_withdrawal(request1.request_id)
            .await
            .unwrap();
    ...
    async fn test_withdrawal_cancellation() {
        ...
        // Update balance
        manager
            .update_available_balance(Address::zero(), U256::from(1_000_000_000_000_000_000u64))
            .await
            .unwrap();
    ...
    async fn test_withdrawal_statistics() {
        ...
        // Update balance
        manager
            .update_available_balance(Address::zero(), U256::from(10_000_000_000_000_000_000u64))
            .await
            .unwrap();
        ...
        for amount in &amounts {
            ...
            manager
                .execute_withdrawal(request.request_id)
                .await
                .unwrap();
        }
        ...
        assert_eq!(
            stats.average_withdrawal_amount,
            U256::from(200_000_000_000_000_000u64)
        );
    ...
    async fn test_multi_token_withdrawals() {
        ...
        manager
            .update_available_balance(eth, U256::from(1_000_000_000_000_000_000u64))
            .await
            .unwrap();
        manager
            .update_available_balance(usdc, U256::from(1000_000_000u64))
            .await
            .unwrap(); // 1000 USDC

        // Withdraw ETH
        let eth_request = manager
            .request_withdrawal(
                U256::from(100_000_000_000_000_000u64),
                eth,
                Address::random(),
