...
mod tests {
    use super::*;
    use revenue_calculator::{JobMetrics, RevenueCalculator};
    ...
    async fn test_revenue_by_period() {
        ...
        let recent = calculator
            .get_revenue_by_period(yesterday - Duration::hours(1), now + Duration::hours(1))
            .await
            .unwrap();
    ...
    async fn test_revenue_by_model() {
        ...
        for (i, model) in models.iter().enumerate() {
            ...
            let revenue = calculator
                .calculate_revenue(job_id, amount, metrics)
                .await
                .unwrap();
