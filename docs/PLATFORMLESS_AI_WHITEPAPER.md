...
The implementation is a **working proof-of-concept**: the attested load path has been **proven end-to-end in integration on real NVIDIA GPU hardware** (encrypted model → attestation → key release → in-memory decrypt → live `llama.cpp` inference on CUDA → secure delete), covered by ~80 tests and hardened across two adversarial review rounds (node version `8.30.0-tee-confidential-inference`). It is not yet wired into the node binary's live request path. The remaining milestones are substituting the current **mock attestation backend** for **real hardware attestation** and wiring the path into live requests (see _Honest status_ below). We describe it here because the architecture is built and the trust model it enables is central to the protocol's privacy story — while being explicit that it is not yet a production guarantee.
...
- **Pipeline built.** The pieces — sealed-container format, signed-policy verification, ephemeral-key remote attestation with cross-binding, Key-Broker key release, tmpfs-only decryption, on-chain hash-binding, secure delete, and capability advertisement — are implemented, fail-closed throughout, and covered by ~80 passing tests.
- **Proven on real GPU hardware, end-to-end in integration.** The complete encrypted-model path runs green on an NVIDIA GPU host from an integration test: encrypt → attest → key release → decrypt to `/dev/shm` → `llama.cpp` inference on CUDA → secure delete. The node binary's live request path does not yet call it; that wiring, and production implementations of the key-broker, policy-source and blob-source seams (mocks today), are part of the remaining milestone.
...
- ✅ Proven end-to-end in integration on real NVIDIA GPU hardware (encrypt → attest → key release → decrypt → `llama.cpp` on CUDA → inference → secure delete); ~80 tests; 2 adversarial review rounds
- ⏳ **Remaining milestone:** swap the mock attestation backend for **real NVIDIA Confidential-Computing attestation** on a Confidential VM (CC-On H100/H200 GPU + Intel TDX or AMD SEV-SNP CPU TEE), build the real key-broker service, and wire the attested load path into the node's live request path — the step that proves a root operator provably cannot read the model or prompt from VRAM
