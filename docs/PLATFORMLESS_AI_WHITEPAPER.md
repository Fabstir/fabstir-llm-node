...
attests to both. That path has now run on real confidential-computing hardware, an Intel TDX
confidential VM with an NVIDIA H200 (September 2026, Section 8.6), and paid testnet sessions
from the Platformless AI app have run on it, but it is not yet the default path sessions take.
Until it is, honest hosts are kept honest by commitments, client-side
checks, and stake, and this document says so rather than calling a commitment a proof of
computation.
...
| Host reads inputs/weights in-use | Confidential computing / TEE: attested key release, model decrypted only inside the confidential VM (proven on real TDX + H200 hardware with paid testnet sessions, not yet the default path, §8.6) |
...
- For workloads that require the host to be blind to inputs as well, two paths apply: **confidential computing via Trusted Execution Environments (TEE)**, the route to in-use privacy, now proven end to end on real confidential-computing hardware (Intel TDX with an NVIDIA H200), with paid testnet sessions run on it, but not yet the default path (Section 8.6); and, today, **client-side pre-processing** — sensitive inputs can be embedded or transformed locally so only derived data (e.g. vectors) reaches the host.
...
### 8.6 Confidential Computing and Trusted Execution Environments (Proven on Hardware, Pre-Production)
...
The implementation runs in the node binary's live request path, and on 23 September 2026 it ran end to end on real confidential-computing hardware: a Phala Cloud confidential VM on Intel TDX with an NVIDIA H200 GPU in confidential-computing mode. An off-node Key Broker Service verified the TDX quote and the H200's attestation through NVIDIA's Remote Attestation Service, both carrying the same one-time challenge, against a provider-signed policy, and only then released the model key. The model, the open-weight Qwen3.8-27B (29 GB), was decrypted inside the confidential VM, loaded onto the H200 and served inference. The same day, paid testnet chat sessions from the Platformless AI app were routed to that host, checkpointed and settled on-chain, with their proofs published to decentralised storage. We describe it here because the architecture is built and proven on the target hardware, and the trust model it enables is central to the protocol's privacy story, while being explicit about what it does not yet prove (see _Honest status_ below) and that it is not yet the default path for paid sessions.

#### What it adds beyond end-to-end encryption

With confidential computing, the model is served from a hardware-isolated **Trusted Execution Environment (TEE)**: a Confidential VM combining a **CPU TEE** (Intel TDX or AMD SEV-SNP, which encrypts and integrity-protects the VM's memory) with an **NVIDIA GPU in Confidential Computing mode**, which NVIDIA designs to protect the PCIe link and GPU memory from the host. What the protocol can state today, and what the hardware run demonstrated, is narrower and exact: the model key is released only against hardware-signed proof that the machine is genuine and running approved software; the model is decrypted only inside a confidential VM whose memory the operator cannot read; and the GPU proves its identity, secure boot and debug state to NVIDIA's verification service. The further claim, that the operator cannot read GPU memory while the model runs, depends on the GPU being in confidential-computing mode, which the signed evidence does not yet settle (see _Honest status_). Trust shifts from *"trust the host operator not to look"* towards *"trust the silicon vendor's hardware root of trust, verified by a remote attestation that the client can check"*.
...
2. ATTESTED KEY RELEASE  generate an ephemeral keypair; gather hardware evidence
                        (GPU report + CPU TEE quote), both bound to one challenge:
                          report_data = SHA-256( ephemeral_pubkey ) ‖ nonce
                          GPU evidence collected under the same nonce
                        ...
                        present in both signed quotes, defeats replay and
                        attestation-relay.
                                  │
                                  ▼
3. DECRYPT-IN-MEMORY     decrypt into RAM-backed tmpfs (/dev/shm, mode 0600) by default;
   + HASH-BIND          a model larger than RAM goes to the CVM's own dstack-encrypted
                        (LUKS) data disk instead, refused unless the device IS LUKS;
                        the container header binds model-id + policy
                        hash (defeats pointer substitution); stream-hash the plaintext
                        and check it against the on-chain SHA-256 model hash; on ANY
                        failure zero it and unlink it; on a clean stop unlink it (a
                        generation may still be reading it, and the pages are CVM
                        memory, freed with the mapping).
```

Every step fails closed: the model key is released **only** after a valid attestation; a node that is not TEE-enabled **refuses encrypted models outright**; and a host advertises the **`tee-attested`** capability — in its on-chain `NodeRegistry` registration and in the encrypted-WebSocket handshake — **only** when TEE enforcement is actually active. The same single flag gates both advertisement and enforcement, so a host cannot claim attested confidentiality it does not honour. Clients can therefore filter host discovery for `tee-attested` nodes and route sensitive workloads only to hosts whose model key was released against hardware-signed proof of a genuine confidential VM running approved software. This composes with the existing layers — E2E encryption still protects transit and storage; model governance (Section 4.2) still pins the model's hash and provider on-chain; the TEE adds in-use confidentiality on top.

#### Honest status

- **Pipeline built.** The pieces — sealed-container format, signed-policy verification, ephemeral-key remote attestation with cross-binding, Key-Broker key release, tmpfs-only decryption, on-chain hash-binding, secure delete, and capability advertisement — are implemented, fail-closed throughout, and covered by more than 300 passing tests across the node and key-broker suites.
- **Proven on real GPU hardware, end-to-end in integration.** The complete encrypted-model path runs green on an NVIDIA GPU host from an integration test: encrypt → attest → key release → decrypt to `/dev/shm` → `llama.cpp` inference on CUDA → secure delete.
- **The CPU half now runs on real silicon.** On a confidential VM with an Intel TDX processor, the node produced a genuine TDX quote, an off-node Key Broker Service verified it against a provider-signed policy and released the wrapped model key in well under a second, and the node decrypted the container and served an inference. The launch measurement and the boot-time registers are pinned; the runtime register is replayed from the VM's event log rather than trusted; the quote's certification data is checked against Intel-signed collateral; and the one-time nonce is required in both halves of the evidence. This is the live request path in the shipped binary, not a simulator and not an integration harness, and the key-broker, policy-source and blob-source seams are now real implementations rather than mocks.
- **The GPU half now runs on real silicon (23 September 2026).** On a Phala Cloud confidential VM pairing Intel TDX with an NVIDIA H200, the key broker verified a genuine TDX quote (Intel TCB status up to date, no advisories) and the H200's own attestation report through NVIDIA's Remote Attestation Service (hardware model, driver and VBIOS versions, secure boot on, debug disabled), and the same one-time challenge was confirmed in both the CPU quote and NVIDIA's signed result. It released the model key on the first boot after the machine's measurements were pinned. The confidential VM's launch and boot measurements were recomputed independently from the provider's published image and matched the hardware quote before they were pinned, rather than simply trusted on first use. The node then checked the decrypted model against its on-chain hash, loaded all 66 layers onto the H200 and served coherent inference through the provider's gateway; on a clean stop it deleted the plaintext. A replayed key request was refused. The broker held the real key ring, not the throwaway ring used in the CPU rounds.
- **What the GPU evidence does and does not settle.** NVIDIA publishes no single field saying "confidential computing is on". The signed per-GPU claims for secure boot and debug state together exclude the developer-tools mode in which the host can inspect GPU memory, and the key broker enforces that pair as a condition of release; the H200 run confirmed both values on real hardware. They do not distinguish confidential computing being switched on from being switched off, and inspecting the H200's signed report on the day found no field that does. Whether a Hopper GPU can produce evidence that NVIDIA's service will pass with confidential computing switched off is not documented either way, and the question is with NVIDIA. Until it is answered, that distinction rests on the platform provider's configuration plus a check inside the measured image, which read confidential computing as on throughout the run. **So this work still does not prove that a root operator cannot read GPU memory**, and that claim is deliberately absent from the rest of this document. This is recorded as an open gap rather than papered over.
- **Paid sessions from the app, on the same hardware.** The same day, with the host's on-chain registration pointed at the confidential VM by hand, the Platformless AI app ran three encrypted testnet sessions on it; for each, the node generated a real zero-knowledge proof, submitted the checkpoint and completed the session on-chain, all confirmed. In a further session with the storage bridge added to the confidential VM, the proof was published to the S5 network: fetched there by its content identifier from an unrelated machine, its SHA-256 equals the proof hash the host committed on-chain.
- **Remaining milestones.** Answer the on-versus-off question above; route paid marketplace sessions to `tee-attested` hosts automatically rather than by hand; bring the video-generation (LTX) weights under the same attested release, since today only the language model's weights are sealed and bound; and cross-check the pinned measurements against the provider's own published values. **Until then, confidential inference should be treated as proven on the target hardware but not yet a production guarantee.**
...
### Confidential Computing — Trusted Execution Environments (Proven on Hardware, Pre-Production)

_A parallel security workstream toward host-blind inference (Section 8.6). Proven end to end on real confidential-computing hardware (Intel TDX with an NVIDIA H200, September 2026); paid testnet sessions from the app have run on it; automatic routing, video weights and one open GPU-evidence question remain._
...
- ✅ Remote-attestation key release — ephemeral keypair + GPU report + CPU-TEE quote, both bound to one challenge (`report_data = SHA-256(pubkey) ‖ nonce`; GPU evidence collected under the same nonce), one-time nonce; Key Broker releases the model key only on a valid attestation
- ✅ Decryption to RAM-backed tmpfs (`/dev/shm`) by default, or for a model larger than RAM to the CVM's dstack-encrypted (LUKS) data disk, refused unless the device is LUKS; the plaintext is zeroed and unlinked on any failure, and unlinked on a clean stop
...
- ✅ Proven end-to-end in integration on real NVIDIA GPU hardware (encrypt → attest → key release → decrypt → `llama.cpp` on CUDA → inference → secure delete); 300+ tests; adversarial review rounds throughout
- ✅ **Real Intel TDX attestation, live:** a genuine hardware quote from a confidential VM, verified by a deployed off-node key-broker service against a provider-signed policy, releasing the model key to the node's live request path
- ✅ **Real NVIDIA H200 attestation, live (23 September 2026):** on a Phala Cloud confidential VM, the key broker verified the TDX quote and the H200's report through NVIDIA's Remote Attestation Service under one challenge, released the key on the first pinned boot, and the open-weight Qwen3.8-27B was decrypted inside the confidential VM, loaded onto the H200 and served inference; measurements recomputed independently before pinning; replay refused
- ⏳ **Open question with NVIDIA:** whether any signed evidence distinguishes confidential-computing mode on from off on Hopper. Until it is answered, this document does not claim that a root operator cannot read the model or prompt from GPU memory
- ✅ Paid testnet sessions from the Platformless AI app run on the TEE host end to end (encrypted chat, zero-knowledge proof, on-chain checkpoint and settlement), with the host routed by hand; with the storage bridge inside the confidential VM, the proof is published to S5 and anyone can fetch it and match it to the hash committed on-chain
- ⏳ Automatic routing of paid sessions to `tee-attested` hosts
- ⏳ Video-generation (LTX 2.3) weights under the same attested release, and video generation inside the confidential VM
...
- ⏳ Confidential computing (TEE): production hardening beyond the September 2026 hardware proof, namely attested-host routing in the SDK, and multi-vendor TEE support
